// ⚠ 已退役，待配对删除（2026-08-22 体检 #58）——生产零调用方。
//   前端 0.23.11 撤制度、0.24.7 清视图；服务端 /api/gates 与 POST /api/config/recommend
//   两处残余已摘（判据：test/gates-recommend-retire.test.js 真起服务真打端点）。
//   现存引用只剩两处测试：test/recommend.test.js（整套 10 项测的就是本模块）与
//   test/relay-scope.test.js:146（拿本模块当「闸那一侧是全局尺」的反向钉）。
//   **单删本文件会当场炸掉那两套**，故本轮保留模块只摘调用点；退役要三处同批处置，
//   见回执 need_coord。删本文件时 package.json 无需改（跑测试.js 按目录扫 *.test.js）。
//
