...
const STATES = [
  '待审', '待派', '待处理', '待重派', '已排期', // 待办态（H116：已排期=放行∧排期双满足，唯一被今时线扫描的待办态——
  //                                            待派/待重派=还没排期或排期被清，不参与派发，2026-08-26 制作人拍板）
  '在途', '初检', '核查', '仲裁', '完成',    // 在途态（完成=出口驻留位：判官全过，停在验收闸前等专项级验收）
  '归档', '挂起', '废弃',                    // 结束态（挂起是唯一可逆终态；删除不存在）
];
const 大态 = { 待办: ['待审', '待派', '待处理', '待重派', '已排期'], 在途: ['在途', '初检', '核查', '仲裁', '完成'], 结束: ['归档', '挂起', '废弃'] };
...
const TRANSITIONS = {
  ...
  待派: ['已排期', '在途', '待审', '废弃', '挂起'], // H116 排期落账→已排期；直派边过渡期保留（散单无粒不走排期）；退回改；废弃；挂起
  待处理: ['待重派', '待审', '完成', '废弃'],   // +完成：定夺「接受」出路（原 待定夺→待验收）            // 分诊回队〔原 执行失败→池〕；返修重拆〔原 H65 执行失败→草稿〕；废弃
  待重派: ['已排期', '在途', '废弃', '挂起'],    // H116 重新排期→已排期；直派边过渡期保留；废弃；挂起
  已排期: ['在途', '待派', '待重派', '废弃', '挂起'], // H116：今时线到点派发→在途；重排清计划退回→待派；重派收回→待重派；废弃；挂起
