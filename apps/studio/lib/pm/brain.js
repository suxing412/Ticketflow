...
function buildCutPrompt(root, cfg, parent, projPath) {
  ...
  return [
    ...
    '调研单（H90）：未知数排查的先行单——调研结论文档本身就是交付物。执行会话必须调异厂评审台（套件/评审台/review.js，喂调研结论 md 全绝对路径）过一遍全员评审，命令口径同方案单：评审团自动发现、异厂全到，少一席要写缺席原因；额度耗尽则弃对应厂并在回执如实记录。回执必带附录三件：①各厂意见原文 ②逐条采纳/驳回（驳回写理由）③由此产生的修订点。验收标准必须含评审台证据项（意见合集 md 绝对路径 + 附录三件齐备）。',
    '方案单（H88）：承重技术方案的设计单——先出方案再动手，方案本身就是交付物。frontmatter 定死：职能: 技术策划、产出物类型: 文档、QA: 开、验收方式: 委托（承重设计不吃⑨「文档类=关」的简检口径，这是明定例外）。正文按五章契约写（章名以协议库 H88 正本为准，至少覆盖 问题与目标／不变量／路径划分与取舍／验证法／实现风险与回滚），缺章即返修。执行会话必须调异厂评审台（套件/评审台/review.js，喂方案 md 全绝对路径）过一遍全员评审——评审团自动发现、异厂全到，少一席要写缺席原因。回执必带附录三件：①各厂意见原文 ②逐条采纳/驳回（驳回写理由）③由此产生的修订点。质检阶段将再评一轮（H90）——执行阶段全量异厂评审、质检阶段全量异厂评审各一轮，额度耗尽则弃对应厂并如实记录。验收标准必须含评审台证据项（意见合集 md 绝对路径 + 附录三件齐备）。',
