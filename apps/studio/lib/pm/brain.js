...
const draftguard = require('./draftguard');
...
// 态集合（三大态改造 2026-08-24）：校准样本要的是「做完的」——完成（判官过、候专项验收）
// 与 归档（落袋）都算；废弃/挂起 不算（没做完，比值是噪声）。
function 历史样本(root, deps = {}) {
  const st = deps.store || store;
  const 单表 = [];
  for (const s of (deps.状态 || ['完成', '归档'])) {
...
function buildCutPrompt(root, cfg, parent, projPath, 校准块) {
  ...
  const 单元 = (cfg.单元 || {});
  return [
    '你是单流的「项目管理」职能（fable 档）。任务：把下面的拍板父单切成单元子单，成单落「待审」候审批。',
    ...
    '⑧附·引擎门禁停闸（H97，2026-08-09 制作人批准，施工令-032 实装）：**验收标准章**里出现 enginectl / unity-test / 受控重建 这类引擎实测特征的单，核查判通过后机器不落「完成」——原位盖「候引擎实证」印候着，待总监确认引擎实测证据确已誊入回执后走「实证放行」才收（特征表可在 studio.config 执行器.引擎门禁.特征 覆盖，缺省即上述三特征；门禁只扫验收标准章）。这是设计内的第二把钥匙，不是故障：切这类单时把验收标准写成「谁跑哪条命令、回执里必须贴哪几个数字」的可复核条目，别只写「测试通过」。',
    '⑦结构归位（H50/H107）：工单树是项管资产，归属按四层链——工单→专项→特性→管线，**只记直接上级**。若本父单无直接上级（无 专项/特性 章），在简报里提出应挂入的既有专项或特性，或建议开新特性（开特性=你提请+总监审核）/开新线（开线是制作人人闸，你只有建议权）；判不出写「呈制作人定归属」',
    ...
    '⑫并发调配语境（施工令-010，2026-08-06 制作人批准）：并发调配权随编制权同规格归你——审检并发（两检初检/核查/仲裁 各自的判官槽数，默认 1）与各池并发上限都由你按积压动态调，走 /api/pm/concurrency（GET 取聚合快照，POST 体 {审检?,零输出分钟?,池?:{codex,claude,deepseek},理由}）。审检并行不是默认（制作人已否决写死并行）：审检积压（初检/核查/仲裁 排队）或 待处理 真堆起来了才调，堆完了调回去。硬顶是成本保险丝、只有制作人能改，越顶一律 400 拒绝——别试。',
    ...
    '归属只记直接上级（H107 四层架构，取代 H51 挂载律）：工单的 frontmatter 写 专项: S-N（或直挂特性时写 特性: F-N），**不要再写 管线: P-N**——管线由 专项→特性→管线 逐级推出来，多处记同一事实正是账实分叉的成因（08-19 审计 148 条分叉过半出自此病）。确无归属的独立杂务才允许两格都留空进散单。存量单里直写的 管线 章读侧仍认（兜底），但新单不许再添。案源：TK-106~116 整条地图施工链漏章堆散单，制作人亲自抓出；口径于 2026-08-23 随 H107 补决议号一并改正。',
    '不设收口单（H56）：全部子单到「完成」后项管自动生成收口报告、专项父单自动转「完成」候验收——制作人的保留签字在父单，一个专项只签一次（H53）。最后一张实现/装配单须含受控重建与全量测试绿的交付责任。',
    '',
    ...
    '专项: <本单所属专项号 S-N；直挂特性的单改写 特性: F-N；确无归属的独立杂务两格都留空>',
    '---',
    '<正文：## 范围 / ## 不要做 / ## 验收标准>',
    '```',
...
//
// 【2026-08-28 补：上一版只修了一半】上面那个状态机只认 ```lang 当嵌套开栏，
// **不带语言标记的裸 ``` 开栏照旧被当成 ticket 收尾**——而模型写命令行示例时几乎都不带语言标记。
// 于是同一个病换个写法立刻复发：TF-14 今日起草，断在「单一出口：」，正是一个裸围栏之前；
// 回头对 TF-3（断「主口：」）、TF-6（断「协议固定…：」）、TK-213（断「输出契约要求「一个」），
// **四张全部恰好断在代码块开栏处**。四张「截断标本」一张都不是模型没写完，全是被解析器切的。
//
// 补法：**区段里最后一个围栏行才是收尾**，之前的一律算正文（含裸围栏开的代码块）。
// 区段 = 从 ```ticket 起，到下一个 ```ticket、或「## 拆单简报」、或文末为止。
//
// 为什么不按「验收标准章之后才算收尾」判：那要求每张单必有三章，而仓里既有判据里
// 就躺着只写背景的短单（draft-fence ⑥）——**拿一条内容约定去当结构解析的依据，
// 会把「格式不合规」变成「解析不出来」，两种病从此分不开**。
// 「最后一个围栏」只依赖结构，不问正文写了什么。
//
// 代价说清楚：模型真被截停、且截停位置恰好在某个代码块收栏之后，这一版会落一张残单
// （旧版是整张丢）。这不是放松——**正文完不完整本就该由起草前置校验闸判**（TF-14 课题），
// 解析器只负责切对边界。让解析器兼职做完整性判定，正是这个病拖了三天的原因。
// 而完全没有围栏的截停（连一个 ``` 都没等到）照旧整张丢弃，「宁缺毋残」在那条路上原样保留。
function parseTickets(text) {
  ...
  const 是开栏 = (i) => /^```ticket\s*$/.test(lines[i]);
  const 是围栏 = (i) => /^```/.test(lines[i]);
  const 是简报 = (i) => /^##\s*拆单简报/.test(lines[i]);

  for (let i = 0; i < lines.length; i++) {
    if (!是开栏(i)) continue;
    // 区段右界：下一个 ```ticket 或 拆单简报 或文末。简报要划出去——
    // 它是块外的散文，里头也可能有代码围栏，不划界就会被当成本块的收尾候选。
    let 界 = lines.length;
    for (let j = i + 1; j < lines.length; j++) {
      if (是开栏(j) || 是简报(j)) { 界 = j; break; }
    }
    // 区段里最后一个围栏行 = 本块收尾
    let 尾 = -1;
    for (let j = i + 1; j < 界; j++) if (是围栏(j)) 尾 = j;
    if (尾 < 0) { i = 界 - 1; continue; }   // 一个围栏都没等到＝真截停，整张丢
    落一张(lines.slice(i + 1, 尾).join('\n') + '\n');
    i = 界 - 1;                              // 从区段末继续；区段右界已保证不会重入本块
    // 【已知窟窿，非本次引入】正文里若**引用** ```ticket（写起草契约的单会这么干），
    // 那一行会被当成区段右界：外层单因此收不到尾而整张丢，内层那个示例反被解析成一张真单。
    // 现行与改前行为一致，故本轮不动它——修法要把「围栏配对」和「区段切分」互相依赖的环解开，
    // 是另一件事。已记进议程，别在这里顺手改：一个看着像两行的改动会把上面九条判据的前提全换掉。
  }
...
function childFm(tk, { id, ids, parentId, 项目, 挂链, 容器管线 }) {
  ...
  return {
    ...
    ...(tk.fm.钉池 || tk.fm.执行池 ? { 钉池: String(tk.fm.钉池 || tk.fm.执行池) } : {}), // 2026-08-26 分家：起草章即意图，翻译成钉池（同 draftFm）
...
function _重排集(root, opts, 全) {
  ...
  const 未了结 = (g) => {
    ...
    try {
      ...
      if (!t) return true;
      // 停靠单的粒不进重排集（议程第 33 条）：它在等一个人的决定，排到几点都跑不了。
      // 2026-08-27 实证：今夜十一粒里九粒挂在停靠单上，复判五轮把它们来回挪了 15~330 分钟，
      // 全是无用功，还让复判每拍都看见「到点却零在途」而反复误诊。
      if (t.fm && t.fm.停靠 === true) return false;
      return ['待审', '待派', '待重派', '已排期'].includes(t.state);
    } catch { return false; }
  };
...
function draftFm(tk, { id, 项目, 粒ID, 依赖, 归属 }) {
  return {
    ...
    ...(粒ID ? { 粒ID: String(粒ID) } : {}),
    // H107：模型自填的直接归属也要透传；排程台账注入仍在下方覆盖它。
    ...(tk.fm.专项 ? { 专项: tk.fm.专项 } : {}),
    ...(tk.fm.特性 ? { 特性: tk.fm.特性 } : {}),
    // 归属注入（2026-08-26 假 100% 案）：同 依赖/粒ID——排程台账是归属的事实源，
    // 放在模型自填（上方 tk.fm.管线）之后＝注入盖过自填。
    ...(归属 || {}),
    // 钉池透传（2026-08-26 分家）：起草稿盖的池章一律翻译成 钉池 落盘——起草时刻不存在运行记录，
    // 模型按 H88 契约写的 执行池 就是意图（工程单 deepseek 成本钉），写口翻译、契约面不动
    // （同 裁决改池 的翻译纪律）。白名单吞字段的老病（依据/管线/依赖 三案在前）不许在钉池上重演。
    ...(tk.fm.钉池 || tk.fm.执行池 ? { 钉池: String(tk.fm.钉池 || tk.fm.执行池) } : {}),
  };
}
...
function closeout(root, cfg, parentId, cb) {
  ...
  const prompt = [
    ...
    '⑤（H69 仪表盘）报告末尾单独一行「审检报告可用性：N」（N=1-5）——评本专项各审检报告（回执里的初检/委托代核章）作为收口材料的可用度：证据链清晰度、返工建议可执行性。',
  ...
  child.on('close', () => {
    ...
    try { require('../relay').发(root, '项管', '收口报告：' + parentId + String.fromCharCode(10) + String.fromCharCode(10) + text.slice(0, 3000)); } catch { /* 信道失败不阻塞 */ }
    cb({ ok: true, 报告: rp });
  });
  try { child.stdin.write(prompt, 'utf8'); child.stdin.end(); } catch { /* close 兜底 */ }
}
...
function replanReview(root, cfg, 触因, cb) {
  ...
  const p = (n) => String(n).padStart(2, '0');
  const 今 = `${now.getFullYear()}-${p(now.getMonth() + 1)}-${p(now.getDate())}T${p(now.getHours())}:${p(now.getMinutes())}`;
  // 现状取数（议程第 42 条，2026-08-28）。
  //
  // 病灶：表头写着「计划 vs 现状」，载荷里**只有计划**——没有放行态、没有单在哪个目录、
  // 没有是否停靠、没有实际起止。模型被要求做计划-现实比较却无现实可比，只能编。
  // 实测四轮里三轮在编：08-27 00:39「实际进度较计划前移超 3 小时」（当夜零执行）、
  // 01:54「长块估时严重虚高致整体前移逾3小时」、03:23 更写出「产线空转 68 分钟」与
  // 「进度前移超 3 小时」自相矛盾的一句，且判词说「整体前移」而实测九粒全部延后、零粒前移。
  // 判「重排」会直接调 schedulePlan 改写真实排程台账 —— 于是虚构结论成为账实。
  // **这不是诊断不准，是审计记录不可信**：事后按 因 字段重建「当时为什么这么排」会得到反向结论。
  const 现状 = (g) => {
    const o = { 放行: null, 所在: null, 停靠: false, 实起: g.实际开始 || null, 实完: g.实际完成 || null };
    if (!g.单号) return o;
    try {
      const t = store.find(root, String(g.单号));
      if (!t) return { ...o, 所在: '单不存在' };
      o.所在 = t.state;
      o.放行 = t.fm.放行 === true;
      o.停靠 = t.fm.停靠 === true;
      o.主办 = t.fm.主办 || null;
    } catch { /* 读不到就留 null——**不许猜**，猜出来的现状比没有现状更坏 */ }
    return o;
  };

  const prompt = [
    '你是单流的「项目管理」职能。产线触发了排期复判（H115）——判断现有排期还成不成立、要不要重排。',
    // 触因改成中性陈述：原文「排期是否过于保守」是**诱导性提问**，先把答案方向给了。
    '触因（**只是现象，不预设结论**；空转可能来自排期，也可能来自放行侧、停靠、依赖或额度）：'
      + String(触因 || '').slice(0, 200),
    '只答判断，输出契约：一段 80 字内理由，然后一个 ```json 代码块：{"决定":"维持|重排","因":"一句话"}',
    '「重排」的门槛：排期与现实的偏差已影响后续单的可信度（整体前移/滞后超 2 小时、依赖链错位、产线将长时间空转）。小偏差维持——排期不是秒表。',
    '',
    '**判断纪律（违反即判无效）**：',
    '① 只依据下方「现状」列作判断。**现状列没给的事实一律不许推断**——',
    '   尤其不许凭空断言「实际进度前移/滞后了多少」：那要有 实起/实完 才说得出，没有就说没有。',
    '② 停靠=true 的粒**不是排期问题**：它被人为摁住等裁决，挪到几点都跑不了。这类粒一律维持原时点。',
    '③ 放行=false 且 所在=待派 的粒卡在放行侧，**不是排期问题**——重排解决不了它，',
    '   该在「因」里说明是放行侧的事，让上游去处置。',
    '④ 标「人工闸」的粒时点是**硬约束**，任何重排都不得挪动它（那是制作人本人要到场的时刻）。',
    '',
    '=== 今时 ===', 今,
    '=== 在排粒（计划 ＋ 现状）===',
    '字段说明：计划=计划开始→计划完成；放行/所在/停靠/实起/实完 是**现状实测值**，null 表示读不到（不是 0 也不是"正常"）',
    在排.map((g) => {
      const s = 现状(g);
      return JSON.stringify({
        粒ID: g.粒ID.slice(0, 8), 题: (g.题 || '').slice(0, 20), 状态: g.状态,
        计划: (g.计划开始 || '') + '→' + (g.计划完成 || ''), 单号: g.单号,
        放行: s.放行, 所在: s.所在, 停靠: s.停靠, 实起: s.实起, 实完: s.实完,
        人工闸: /制作人|拍板|签字|验收|读屏|亲自/.test(String(g.题 || '')) || undefined,
      });
    }).join('\n') || '（无）',
  ].join('\n');
  const cmd = cli();
...
function adjudicateReferral(root, cfg, id, cb) {
  ...
  // 停靠单不裁（议程第 33 条）：裁决的三个出口里「改单/改池」都会带放行重新可派，
  // 而停靠的含义正是「等一个人的决定，先别推上产线」。
  // 案源 2026-08-26 两次同分钟对冲：总监 16:51 撤放行停靠 TK-187，项管 16:52 裁决带放行又开；
  // TK-207 18:17/18:18 同样。裁决侧读不懂停靠语义，于是职权在同一分钟里互相抵消。
  // 这里早退而非静默跳过：拒绝要留痕，才看得出「这张单没被裁是因为它停着」。
  if (t.fm && t.fm.停靠 === true) {
    return cb({ ok: false, error: `${id} 已停靠，不进项管裁决（候人裁：${String(t.fm.停靠因 || '').slice(0, 80)}）`, 停靠: true });
  }
  let receipt = '';
  ...
  child.on('close', () => {
    ...
    try { require('../relay').发(root, '项管', `评估回呈裁决 ${id}：${v.处置}——${v.说明 || ''}`); } catch { /**/ }
    if (v.处置 === '上呈') {
    ...
    store.update(root, id, (fm, t2) => {
      const patch = { 放行: true };
      // 裁决改池落 钉池 不落 执行池（2026-08-26 分家）：这是刻意路由指令，回队不许被清章销掉；
      // 契约面不动（模型照旧回 执行池 字段），写口翻译成钉章。
      if (v.处置 === '改池' && ['claude', 'codex'].includes(v.执行池)) patch.钉池 = v.执行池;
      if (v.处置 === '改单') {
        const f = v.字段修改 || {};
...
// 制作人层提需求 → 项管起草单张工单（待审态）→ Claude 审 → 定稿放行。审批与起草分离。
...
// opts.粒ID（施工令-040 第 6 条，可选）：这次起草兑现的排程计划粒。落单（待审态）成功后
...
function buildDraftPrompt(cfg, 需求, projPath, 校准块) {
  ...
  return [
    ...
    '引擎门禁停闸（H97，2026-08-09 制作人批准，施工令-032 实装）：验收标准章里写了 enginectl / unity-test / 受控重建 这类引擎实测特征的单，核查判通过后机器不落「完成」——原位盖「候引擎实证」印候着，待总监确认实测证据誊入回执后走「实证放行」才收。这是设计内的第二把钥匙，不是故障：这类验收标准要写成「谁跑哪条命令、回执里必须贴哪几个数字」的可复核条目，别只写「测试通过」。',
    FIELD_RULES,
    '输出契约与拆单相同：一个 ```ticket 代码块（title/单型/职能/产出物类型/优先级/QA/验收方式/预计时间/预计token/依赖（需求点名了就写，否则留空）/管线 + --- + 正文三章）。之后一段「起草说明」：盘点发现+边界取舍。',
    '归属只记直接上级（H107，取代 H51 挂载律；案源 TK-106~116，起草单尤重）：单张起草单没有父单可继承归属，漏写就必落看板「散单」行——frontmatter 写 专项: S-N（或直挂特性时写 特性: F-N），**不要写 管线: P-N**，管线由 专项→特性→管线 逐级推出来（推导链见 lib/pipelines.pipelineOf）。确无归属的独立杂务才允许留空。',
    '', 校准块 || estimate.提示词块(null),
...
function draftTicket(root, cfg, 需求, projPath, cb, opts) {
  ...
  child.on('close', () => {
    ...
    if (!落.ok) return cb(落);
    const 校验 = draftguard.查草稿({ fm, body: tk.body, 需求, 项目, cfg });
    if (!校验.ok) {
      try {
        require('../journal').append(root, `起草校验闸拦截（拟落 ${nid}）：违规 ${校验.违规.map((x) => x.型).join('、')}——未落盘、号未消耗`);
      } catch { /* 留痕失败不阻塞 */ }
      return cb({ ok: false, error: 校验.违规.map((x) => x.说明).join(String.fromCharCode(10)), 违规: 校验.违规 });
    }
    ...
    try {
      require('../relay').发(root, '项管', '受托起草：' + nid + ' ' + fm.title + '（待审区候 Claude 审）'
