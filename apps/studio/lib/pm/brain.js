...
function buildCutPrompt(root, cfg, parent, projPath) {
  ...
  return [
    ...
    '调研单（H90）：未知数排查的先行单——调研结论文档本身就是交付物。**产物路径口径（施工令-020）：调研结论一律落 `Docs/SLG/调研方案/<题目>.md`**（竞品横评落 `Docs/SLG/竞品分析/`），两者同挂监制台 wiki「调研方案」分区；产出物路径写别处即返修。执行会话必须调异厂评审台（套件/评审台/review.js，喂调研结论 md 全绝对路径）过一遍全员评审，命令口径同方案单：评审团自动发现、异厂全到，少一席要写缺席原因；额度耗尽则弃对应厂并在回执如实记录。回执必带附录三件：①各厂意见原文 ②逐条采纳/驳回（驳回写理由）③由此产生的修订点。评审台已行**红队立场卷制**（H91/施工令-019）：三卷（可行性红队/不变量红队/成本红队）按席序轮换派发、单厂在席时独领全部三卷，每卷必须尝试构造具体失败场景，构造不出要明写「未能构造击杀」——该声明本身即通过证据。验收标准必须含评审台证据项（意见合集 md 绝对路径 + 附录三件齐备 + 击杀结论：各厂领卷与击杀/未杀条数，缺席致落空的立场卷如实标注）。',
    '方案单（H88）：承重技术方案的设计单——先出方案再动手，方案本身就是交付物。frontmatter 定死：职能: 技术策划、产出物类型: 文档、QA: 开、验收方式: 委托（承重设计不吃⑨「文档类=关」的简检口径，这是明定例外）。正文按五章契约写（章名以协议库 H88 正本为准，至少覆盖 问题与目标／不变量／路径划分与取舍／验证法／实现风险与回滚），缺章即返修。执行会话必须调异厂评审台（套件/评审台/review.js，喂方案 md 全绝对路径）过一遍全员评审——评审团自动发现、异厂全到，少一席要写缺席原因。回执必带附录三件：①各厂意见原文 ②逐条采纳/驳回（驳回写理由）③由此产生的修订点。质检阶段将再评一轮（H90）——执行阶段全量异厂评审、质检阶段全量异厂评审各一轮，额度耗尽则弃对应厂并如实记录。评审台已行**红队立场卷制**（H91/施工令-019）：三卷（可行性红队/不变量红队/成本红队）按席序轮换派发、单厂在席时独领全部三卷，每卷必须尝试构造具体失败场景，构造不出要明写「未能构造击杀」——该声明本身即通过证据。验收标准必须含评审台证据项（意见合集 md 绝对路径 + 附录三件齐备 + 击杀结论：各厂领卷与击杀/未杀条数，缺席致落空的立场卷如实标注）。',
