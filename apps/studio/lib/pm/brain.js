...
function buildCutPrompt(root, cfg, parent, projPath) {
  ...
  return [
    ...
    '⑪用工语境（H85，2026-08-06 制作人裁决）：agent 编制数据与调整权在你（项管）手上——监制台参数页已无编制管理区，改编制走 /api/pm/roster。池路由按编制快照与额度实况定：某职能的编制全挂在被额度锁冻结的池上时，派发引擎会临时改挂到可用池（自动动作只此一种，会留 fm.临时改池 + 台账）；其余用工调整由你显式发起，别指望自动。',
    '⑨分级检审（2026-08-05 制作人批准）：QA 字段按单型定死——代码/装配/程序类=开（全检）；文档/wiki/调研/工程类=关（简检，仅核查两检）；验收方式=保留 的品味单免检直达制作人。不逐单裁量。',
    '',
    '=== 单型库（只从五型选：调研单/实现单/装配单/修复单/工程单）===',
...
const FIELD_RULES = '字段规范（补：单型=工程单 时 执行池 必须= deepseek，QA=关）（逐字遵守，违者返修）：职能 只能取 策划/程序/美术/QA/装配 五者之一，不得加括号后缀；优先级 只能取 P0/P1/P2/P3；QA 只能取 开/关；需求若点名依赖单号或执行池，frontmatter 必须含 依赖:/执行池: 字段原样带上。正文三章（背景/执行内容/验收标准）必须写在 ticket 代码块内部、frontmatter 的第二个 --- 之后——写在代码块外会被解析丢弃（TK-86 空壳案）。';
