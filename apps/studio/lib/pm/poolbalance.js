...
const 默认参数 = {
  ...
  拒因静默小时: 24,       // 恒定拒因（品味锁这类不会自己变的）在此窗口内只记一次账，不逐拍刷
};
...
// H108 换轨（旧：草稿/待投/池/在途/质检）：待审=原草稿；待派=原待投+池；待重派=分诊回队重派位
// （旧机器里回队单落回「池」，池在旧活态表里，故待重派同列）；初检/核查/仲裁=原「质检」拆出的判官链——
// 委托验收的品味单/美术单在这三站上都还没落定，判官席的锁必须罩全链，不能只认初检一站。
const 活态 = ['待审', '待派', '待重派', '已排期', '在途', '初检', '核查', '仲裁']; // H116：已排期（放行∧有排期，候今时线派发）的品味单同样要锁位
...
function 历史Of(events, 参, 现在) {
  ...
  return { 最近, 冷却至, 现在: now, 事件们: (Array.isArray(events) ? events : []), 冷却中: (池) => (冷却至[池] || 0) > now };
}

// 恒定拒因去重（2026-08-20 补）：同指纹在静默窗内已记过就别再记。
// 纯函数，不碰盘——事件们由 历史Of 带出来，与迟滞/冷却共用同一份台账切片。
// 为什么按「指纹」而不按「位」：同一位可能因不同原因被拒（品味锁 vs 人工覆盖），
// 那是两件事，各自该有各自的第一次；同因同目标才是重复喊话。
function 已记过(事件们, 指纹, 现在, 静默小时) {
  const 窗 = Math.max(0, Number(静默小时) || 0) * 3600000;
  if (!窗) return false;
  const now = 现在 || Date.now();
  for (const e of (Array.isArray(事件们) ? 事件们 : [])) {
    if (!e || e.指纹 !== 指纹) continue;
    const t = Date.parse(e.t || '');
    if (Number.isFinite(t) && now - t < 窗) return true;
  }
  return false;
}
...
// 执行失败单摘（回退判定的输入）。H108：执行失败目录并入 待处理（原 执行失败→待处理）——
// 待处理里混着定夺类单，靠 失败时间 字段区分：没有失败时间的单在 回退判定 里 ft 解析不出即被排除。
function 失败单摘(root) {
  const store = require('../core/store');
  let list = [];
  try { list = store.list(root, '待处理'); } catch { list = []; }
...
function 巡检(root, cfg, 读数, opts = {}) {
  ...
  for (const d of 决策({ 位们: 建位们(), 读数: 读数 || {}, 参, 历史, 覆盖: 参.人工覆盖 })) {
    ...
    if (d.动作 === '拒') {
      // 自动面拒因去重（2026-08-20，制作人指认）：**恒定拒因不该逐拍记账**。
      // 实证：品味锁把「执行·美术 claude→codex」拒了 265 次，从 08-11 一路刷到 08-20——
      // 九天的死循环在参数页上只表现为一条不动的红标（那页只显示最近 5 条），
      // 台账则被这一条刷成噪音（271 条池衡事件里 265 条是它）。
      // 判据：同一 位×因类×拟切目标 在窗口内已记过就不再记，只在**状态变化**时落一条。
      // 孤粒早有去重（ledger-sync 报孤粒按粒去重），池衡漏了这一手，本次补齐。
      // 注意去重的是**记账**不是**判断**——判断照跑，拒还是拒，只是不再重复喊。
      const 指纹 = ['池衡拒绝', d.位, d.因类, d.拟切 ? d.拟切.到 : ''].join('|');
      if (!已记过(历史.事件们, 指纹, 现在, 参.拒因静默小时)) {
        记(root, '池衡拒绝', { 位: d.位, 因类: d.因类, 由: '项管自动', 操作者: '项管', 从: d.当前池, 到: d.拟切 ? d.拟切.到 : null, 因: d.因, 指纹 });
      }
      out.拒.push(d);
      continue;
    }
