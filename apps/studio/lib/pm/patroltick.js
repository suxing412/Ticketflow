...
function 造巡检拍(取ROOT, 取cfg, opts = {}) {
  ...
  return function 拍() {
    ...
    try {
      ...
      let 队列长 = 0; // P0-3：排队等派的张数（待派/待重派），排期估算的另一半输入
      const 扫到 = 逐狗('在途扫描', () => {
        // 在途容量只数「在途」目录态（三大态改造 2026-08-24）：完成 是在途大态的出口驻留位
        // （判官全过、候专项级验收），**不占执行槽**，不入并发账；初检/核查/仲裁 占的是判官槽，同样不入。
        ...
        // 队列长语义=「排队等派」（三大态改造 2026-08-24，原口径 待投+池）：待派（池 已并入——
        // 放行成为 fm 标记不再是目录跳变）、待重派（分诊/挂起复活后重新排队）与 已排期（H116：
        // 排期落账等今时线到点，正是排队的主形态）都在排；
        // 待审/待处理 不算——前者还没过审，后者等的是裁决不是派发。容器单（战役/专项）过滤口径与在途同
        for (const 态 of ['待派', '待重派', '已排期']) {
