...
//   ② 只前进不倒退：差量只提状态机允许的**前向**动作。粒已终态、或工单退回上游（H65 返修同号回待审），
...
// ---- 状态映射（工单 12 态 → 粒五态；H108 三大态状态机 2026-08-24）----
// 粒状态机（计划/起草中/已成单/完成/撤销）本身**一格未改**，改的只是单侧的压缩口径。逐态语义：
//   待审/待派 → 起草中：单已起草、还没派发。待派 不认成已成单的理由与原「待投」同一条——
//     派发钩子（schedule.挂钩派发）落在 待派→在途 那一刻，这里若抢先认已成单，同一张单会被
//     两条路各成一次单。原「池」并入待派后 放行 只是 fm 标记，不再另占一态，口径不受影响。
//   待处理/待重派 → 已成单：两态都是**派发之后**的回流位（执行失败/三振/仲裁上交→分诊→回队），
//     单早已发出去过（原「执行失败/待定夺 → 已成单」同判）；粒不退回起草中——账不倒着写。
//   在途/初检/核查/仲裁 → 已成单：审检链目录化（原 质检/待验收 一段）后仍是「单在流程中现身」。
//   完成 → 完成：新「完成」未经专项级验收（判官过而已，H108 出口驻留位）；但**专项内部**
//     子单到完成即算「做完等关账」（H110），排程粒账收口用的正是这个内部口径。
//   归档 → 无因=完成 / 带因=撤销：沿用旧「已归档」分岔——无因归档是正常交付后的整理性归档；
//     历史「归档原因:废弃」的单留在归档不重分类（不改史），靠这条分岔照旧判成撤销。
//   废弃 → 撤销：原「归档原因:废弃」的独立目录化，不算交付。
//   挂起 → **粒不动**（返回 null）：挂起是唯一可逆终态（挂起→待重派 可复活），此刻把粒推到
//     完成/撤销 都是提前定论；停在现状，等单复活或废弃后再随单走。
const 单态到粒态 = {
  待审: '起草中', 待派: '起草中',
  待处理: '已成单', 待重派: '已成单',
  在途: '已成单', 初检: '已成单', 核查: '已成单', 仲裁: '已成单',
  完成: '完成',
};
function 目标状态(t) {
  if (!t) return null;
  if (t.state === '归档') return (t.fm || {}).归档原因 ? '撤销' : '完成';
  if (t.state === '废弃') return '撤销';
  return 单态到粒态[t.state] || null; // 挂起不在表里 → null：粒保持现状
}
...
function 差量(快照, 粒们, 专项们) {
  ...
  for (const t of 单们) {
    ...
    const w = 走法(g.状态, 到);
    if (!w) continue; // 不可达＝要往回走（返修同号回待审等）：只前进不倒退
    动作.push({
  ...
  // 哨兵号不是单号（2026-08-22 体检 #62）：对账把「活早已落地、但从没开过工单」的粒回填成
  // 「（无单·直接落码）」以过 schedule 的单号校验。它天生查无此单，54 秒后就被判成孤粒——
  // 三粒一次性炸出三条 台账孤粒 告警。这类号是**显式的无单声明**，不是挂错的单号，不许报孤粒。
  // 表放在这里而不是 schedule：孤粒判定是唯一会把「查无此单」当异常的地方，闸只此一处可查。
  const 无单哨兵 = new Set(["（无单·直接落码）"]);
  const 异常 = [];
  for (const g of 粒们 || []) {
    if (!g || !g.单号 || 无单哨兵.has(String(g.单号)) || 按号.has(String(g.单号))) continue;
    异常.push({
