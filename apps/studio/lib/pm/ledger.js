...
// 成本归集的「完成数」口径（H108 后按语义逐个判，不是无脑改名）：
// 这一格答的是「这个容器名下做完了几张」——专项内部子单到 完成 即算「做完等关账」（H110），
// 故 完成+归档 都计（归档=验收过；完成=判官全过候验收，都是「做完了」）。
// 废弃/挂起 不计：砍掉的和冻着的不是做完。与 schedule-view 的依赖解除口径（DS-9 只认归档）
// 是两把不同的尺——那边判「下游能不能开工」，这边数「花出去的活成了几张」，语义不同不共尺。
const 终态 = ['完成', '归档'];
const 容器类 = ['战役', '专项'];
...
const 桶表 = {
  ...
  // 排程重排（2026-08-20 补）：漏它一格，排期这项核心动作就落进「其他」桶——
  // 桶开出来了却把最该看见的那类漏在外面，比不开桶更坏（项管页实弹当场抓到）。
  排程登记: '排期', 排程转移: '排期', 排程调整: '排期', 排程重排: '排期',
  估时校准: '估时校准',
