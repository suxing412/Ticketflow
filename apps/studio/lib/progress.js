...
function compute(input) {
  ...
  let 段内 = 0;
  if (打点) 段内 = 打点.k / 打点.n;                       // 有打点：执行段内按 k/n
  else if (estMs > 0 && 有起时) 段内 = 耗时 / estMs;       // 回落时间口径
  // 无预计时间的单阶段内不插值 → 段内 0，直接显示阶段锚点
