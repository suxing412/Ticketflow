...
async function 哨兵(root, cfg, opts = {}) {
  ...
  // 三态分言（2026-08-28 修，议程第 43 条）。
  //
  // 病灶：原文案不分「续得晚」与「续不上」，临期时一律写「请重登 … 自续已试：跑通了但 expiresAt 没动
  // ——CLI 没触发续期」，读上去像机制失灵。**实据恰恰相反**：
  //   [2026-08-27 10:37] OAuth 自续成功 +8h（第 1 发探针；**原剩 -3 分钟** → 现剩 479 分钟）
  // ——它是在 token 已过期 3 分钟之后才续上的，靠的是「打满后每 N 分钟放行一发」的重挂。
  // 临期时 CLI 通常不刷新一张仍然有效的 token，那声「跑通了但没动」是**正常现象不是故障**。
  //
  // 代价不是嘴上的：08-27 一日三响（02:19/10:07/18:22），总监两次据此误判
  // （判「会一直死到早上」、判「不登可能就断」），两次都被自续打脸。
  // 文案让人以为该立刻人工干预，而实际该做的是等它自己续。
  //
  // 分法：临期＝正常，说清「到期后自动重试」；过期未尽＝重挂进行中；过期已尽＝这才真该人工登。
  const 续败 = 自续果 && 自续果.尝试 && !自续果.成功;
  const 已尽 = !!(自续果 && 自续果.已尽);
  // 探针拿到 401/unauthorized，跟「跑通了但 expiresAt 没动」**不是一回事**：
  // 前者是服务端已经不认这张 token 了——哪怕 expiresAt 还说它有效，那也是死的，等下去等不回来。
  // 后者才是临期的正常现象。不分这两者，就会把真故障混进「不用动手」里静默掉。
  const 硬拒 = !!(续败 && /401|unauthor|invalid|revoked|expired/i.test(String(自续果.因 || '')));
  const 该人工 = s.态 === '未登录'
    || 硬拒
    || (s.态 === '过期' && (已尽 || (自续果 && 自续果.次数 >= 2)));

  const 文 = s.态 === '临期'
    ? (硬拒
      ? `OAuth 临期但**探针被硬拒**（${String(自续果.因).slice(0, 80)}）：${s.因}`
        + `——服务端已不认这张 token，expiresAt 说有效也没用，等不回来，**该人工登**`
      : `OAuth 临期：${s.因}——低于 ${临期分钟} 分钟即报。**这一态通常不需要动手**：`
        + `CLI 一般不刷新仍有效的 token，真正的续期发生在到期之后（重挂机制）。`
        + `到期前后会有几分钟真空，期间 claude 池拒派（不空烧）。想避开真空可手动重登。`)
    : s.态 === '过期'
      ? (该人工
        ? `OAuth 已过期且自续连败${自续果 ? `（${自续果.次数} 发）` : ''}：${s.因}——**这次真该人工登了**，claude 池会话随时 401`
        : `OAuth 已过期，自续重挂中${自续果 ? `（第 ${自续果.次数} 发）` : ''}：${s.因}——派发预检拒派，不空烧；续上即自动恢复`)
      : `OAuth 未登录，请重登：${s.因}——claude 池（含判官三席）无凭据可用`;

  // 一键重登只在**真该人工**时给：临期/重挂中也甩一条命令出来，等于每次都在喊「快动手」，
  // 喊多了人就不看了——今天三响两误判，就是这么来的。
  const 配方段 = 该人工 ? `｜一键重登：${登录配方(cfg)}` : '';
  // 自续结果照旧附上（可查），但临期态下明说它「属正常」，不再让人自己去猜
  const 自续段 = 自续果
    ? `｜自续：${自续果.因}${(s.态 === '临期' && 续败) ? '（临期未续属正常，见上）' : ''}`
    : '';
  const 全文 = `${文}${配方段}${自续段}`;
