...
app.get('/api/board', (req, res) => {
  ...
  for (const s of store.STATES) out[s] = snap[s].map((t) => ({
    ...
    执行池: t.fm.执行池 || null, // 2026-08-26 优化包：甘特悬浮卡「现状」要报单在哪个池上跑
    // 审检章结论两格（2026-08-26 制作人起夜案「审检三席只显示待命」）：在途页要按
    // 候初检/候核查/候人裁 分桶画驻留队列——只下发结论字符串，不下发整章（章体是审检席的账）。
    初检结论: (t.fm.初检 && t.fm.初检.结论) || null,
    代核结论: (t.fm.代核 && t.fm.代核.结论) || null,
...
app.post('/api/pm/draft', (req, res) => {
  ...
  require('./lib/pm/brain').draftTicket(ROOT, cfg, 需求, projPath, (r) => {
    ...
    // 归属透传（2026-08-26 假 100% 案）：粒.上级（F-n/S-n/P-n）翻成工单直接归属档随起草落盘，
    // 孵化单从此进得了工单页管线聚合的分母。无上级/认不出前缀＝散单，照旧不带。
    归属: require('./lib/pm/brain').归属自上级(兑现粒 && 兑现粒.上级) });
...
const 排程动作 = {
  ...
  // 表态（H112 今时线制 · 甲组写口的 HTTP 入口）：越线强制二选一在 lib 层校验；
  // **关闸分支在这一层**（DS-11 外审吸收）：产线闸合着时手动派发被拒——闸是全局的，写口不许绕。
  表态: (b) => {
