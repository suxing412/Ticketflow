...
(function () {
  ...
  function 拼树(数据) {
    ...
    for (const g of (数据.粒 || [])) {
      ...
      // 行名优先单题（2026-08-26 制作人 185 案：树列显示粒登记题「面板卡顿根治」而看板显示
      // 单真题「自交扫描移出面板重绘」——粒题是排期意图的旧笔，成单后单题才是身份；粒题留悬浮卡）
      const 册题 = g.单号 && 数据.单册 && 数据.单册[String(g.单号)] && 数据.单册[String(g.单号)].题;
      const n = 造('工单', g.粒ID, 册题 || g.题); n.粒 = g;
      (亲 ? 亲.子 : 伪组.子).push(n);
    }
  ...
  function 行HTML(r, st) {
    ...
    if (!窗.空) {
      if (折) {
        ...
        if (s && !活条 && !n.史 && !(n.实段 && n.实段.定格)) { // 定格单归覆盖分支独画（2026-08-26 制作人抓的 185 双画案：漏排定格致计划条+红段与覆盖条并存）
