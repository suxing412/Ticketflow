...
(async () => {
  ...
  // ── ⑩ H112/H113（2026-08-24 落实表 P2 批次）：刻钟计划 / 表态写口 / 未排期粒 ──
  // 计划粒度收细到 15 分钟（触发粒度分钟制的数据层前提）；越线表态强制二选一；重排类别五类；
  // 未排期清单给 DS-12 防僵尸单闸做判据。全部走行为面：登记/重排/表态真写真读，不 grep 源码。

  await t('P2-1 刻钟计划：YYYY-MM-DDTHH:mm 15 分对齐可登记，纯日期兼容，混形态起止按 00:00 可比', async () => {
    const root = makeRoot();
    const g = 登记一(root, { 计划开始: '2026-09-01T09:15', 计划完成: '2026-09-01T18:45', 因: '刻钟首排' });
    const now = S.取(root, g.粒ID);
    assert.equal(now.计划开始, '2026-09-01T09:15', '刻钟形态原样落盘，不归日');
    assert.equal(now.计划完成, '2026-09-01T18:45');
    assert.deepEqual([now.基线开始, now.基线完成], ['2026-09-01T09:15', '2026-09-01T18:45'], '首次排期基线同形态跟着立');
    for (const mm of ['00', '15', '30', '45']) assert.ok(S.规范计划时刻(`2026-09-01T10:${mm}`).值, `对齐刻度 10:${mm} 应过`);
    // 纯日期照旧（存量形态兼容读）
    const g2 = 登记一(root, { 题: '纯日期', 来源: 'd', 计划开始: '2026-09-02', 计划完成: '2026-09-03', 因: 'x' });
    assert.equal(S.取(root, g2.粒ID).计划开始, '2026-09-02');
    // 混形态起止：开始带时刻、完成纯日期（=当日 00:00）→ 10:00 > 00:00 应拒
    const r = S.登记(root, [粒模板({ 题: '倒挂', 计划开始: '2026-09-05T10:00', 计划完成: '2026-09-05', 因: 'x' })], '总监');
    assert.ok(!r.ok && /计划开始须早于或等于计划完成/.test(r.error), '纯日期按当日 00:00 参与比较：' + r.error);
    // 同日 T00:00 与纯日期等价，不算倒挂（字符串比会在这里误判，必须走毫秒轴）
    assert.ok(S.登记(root, [粒模板({ 题: '零点等价', 计划开始: '2026-09-06T00:00', 计划完成: '2026-09-06', 因: 'x' })], '总监').ok,
      'T00:00 与纯日期同刻，不该被当成倒挂');
  });

  await t('P2-1 刻钟校验：15 分不对齐拒 / 时刻不存在拒 / 秒级 ISO 沿旧口径归日 / 工期算法兼容两形态', async () => {
    const root = makeRoot();
    const 坏 = [
      ['2026-09-01T10:07', '15 分对齐'],
      ['2026-09-01T10:20', '15 分对齐'],
      ['2026-09-01T25:00', '时刻不存在'],
      ['2026-02-30T10:15', '不是真实存在的日期'],
      ['2026-09-01 10:15', 'ISO 日期串'],
    ];
    for (const [v, 词] of 坏) {
      const r = S.登记(root, [粒模板({ 计划完成: v, 因: 'x' })], '总监');
      assert.ok(!r.ok, `应拒：${v}`);
      assert.ok(r.error.includes(词), `拒因应含「${词}」，实际：${r.error}`);
      assert.equal(S.现态(root).length, 0, '整批未写入：一条不合法则一条都不落');
    }
    // 秒级 ISO 归日不受刻钟制约束（老调用方传 toISOString 的旧行为一格不破，10:37 也不拦——存进去的只有日）
    assert.equal(S.规范计划时刻('2026-09-01T10:37:00.000Z').值, '2026-09-01', '秒级串沿旧口径归日');
    // 工期算法兼容两形态：刻钟粒照样按日归一参与延期/超期算术
    const 判 = S.工期判定({ 状态: '计划', 计划完成: '2026-08-24T18:30', 基线完成: '2026-08-20T09:00' }, { 今日: '2026-08-27' });
    assert.deepEqual([判.延期天, 判.超期, 判.超期天], [4, true, 3], '刻钟形态按日归一进工期判定');
    assert.equal(S.日差('2026-08-20T23:45', '2026-08-22'), 2, '日差兼容刻钟形态');
  });

  await t('P2-1 计划毫秒：两形态唯一比较轴——纯日期=当日 00:00，刻钟按分钟，缺/坏为 null', async () => {
    assert.equal(S.计划毫秒('2026-09-01'), S.计划毫秒('2026-09-01T00:00'), '纯日期按当日 00:00 起算（比较函数统一的正身）');
    assert.equal(S.计划毫秒('2026-09-01T09:15') - S.计划毫秒('2026-09-01'), 555 * 60000, '09:15 = 当日起点后 555 分钟');
    assert.ok(S.计划毫秒('2026-09-01T23:45') < S.计划毫秒('2026-09-02'), '刻钟与纯日期跨形态可直接比大小');
    assert.equal(S.计划毫秒(null), null, '缺值 null');
    assert.equal(S.计划毫秒('坏日子'), null, '坏值 null，不拿 0 冒充 1970');
  });

  await t('P2-1 重排吃刻钟：同日改时刻算变更（延期按日=0），不对齐拒，倒挂拒，审计带时刻不丢', async () => {
    const root = makeRoot();
    const g = 登记一(root, { 计划开始: '2026-09-01T09:00', 计划完成: '2026-09-01T12:00', 因: '首排' });
    const r = S.重排(root, { 粒ID: g.粒ID, 计划完成: '2026-09-01T18:30', 因: '同日往后挪半天', 操作者: '项管', 预期版本: 1 });
    assert.ok(r.ok, r.error);
    assert.equal(S.取(root, g.粒ID).计划完成, '2026-09-01T18:30', '同日改时刻是真变更，不该被「无变化」拦');
    assert.equal(r.延期天, 0, '同日内挪时刻，延期按日算 = 0');
    assert.deepEqual(r.原计划, { 开始: '2026-09-01T09:00', 完成: '2026-09-01T12:00' }, '原计划→新计划审计带时刻');
    assert.match(S.重排(root, { 粒ID: g.粒ID, 计划完成: '2026-09-01T10:07', 因: 'x', 操作者: '项管', 预期版本: 2 }).error, /15 分对齐/, '重排口同样把 15 分对齐关');
    assert.match(S.重排(root, { 粒ID: g.粒ID, 计划开始: '2026-09-01T19:00', 因: 'x', 操作者: '项管', 预期版本: 2 }).error, /计划开始须早于或等于计划完成/);
    assert.equal(S.取(root, g.粒ID).版本号, 2, '坏刻钟全拒，现态不动');
  });

  await t('P2-3 表态：今时线强制二选一（无需调整/第三值拒且留痕），其它触发源无需调整必带因', async () => {
    const root = makeRoot();
    const g = 登记一(root, { 计划开始: '2026-08-20', 计划完成: '2026-08-22', 因: '排' });
    const 试 = (p) => S.表态(root, { 粒ID: g.粒ID, 预期版本: 1, 操作者: '项管', ...p });
    assert.ok(!S.表态(root, { 粒ID: 'no-such', 预期版本: 1, 触发源: '今时线', 决定: '派发', 操作者: '项管' }).ok, '粒不存在拒');
    const 越 = 试({ 触发源: '今时线', 决定: '派发', 操作者: '美术' });
    assert.ok(!越.ok && 越.越权, '表态权在项管/总监（越线债消费是项管的活）');
    const 冲 = 试({ 预期版本: 99, 触发源: '今时线', 决定: '派发' });
    assert.ok(!冲.ok && 冲.冲突 && 冲.现态.版本号 === 1, 'CAS 管得住表态，拒了要把现态交回去');
    assert.match(试({ 决定: '派发' }).error, /触发源/, '触发源必填');
    const 三 = 试({ 触发源: '今时线', 决定: '无需调整', 因: '想拖着' });
    assert.ok(!三.ok && /二选一/.test(三.error), '越线不许「无需调整」——强制二选一：' + 三.error);
    assert.ok(S.事件流(root).some((e) => e.事件类型 === '拒绝' && /二选一/.test(e.因)), '第三值企图要留痕');
    assert.match(试({ 触发源: '今时线', 决定: '躺平' }).error, /二选一/, '未知决定同拒');
    assert.match(试({ 触发源: '巡检', 决定: '无需调整' }).error, /必须带因/, '非越线的无需调整必带因');
    const r = 试({ 触发源: '巡检', 决定: '无需调整', 因: '计划本就排在下周' });
    assert.ok(r.ok && r.动作 === '无需调整', r.error);
    const now = S.取(root, g.粒ID);
    assert.equal(now.版本号, 1, '无需调整是审计痕，不顶版本（在途调用方不被 CAS 误伤）');
    assert.equal(now.计划完成, '2026-08-22', '现态一格不动');
    const e = S.事件流(root).find((x) => x.事件类型 === '表态');
    assert.ok(e && e.触发源 === '巡检' && e.决定 === '无需调整' && e.因 === '计划本就排在下周', '表态事件带触发源/决定/因：' + JSON.stringify(e));
  });

  await t('P2-3 表态-派发：禁带新计划，只落表态痕不动状态机，回单号给消费方走 lifecycle', async () => {
    const root = makeRoot();
    const g = 登记一(root, { 单号: 'TK-500' }); // 触发形态：粒有单号且单在待派/待重派
    const 拒发 = S.表态(root, { 粒ID: g.粒ID, 预期版本: 1, 触发源: '今时线', 决定: '派发', 新计划完成: '2026-09-09', 操作者: '项管' });
    assert.ok(!拒发.ok && /禁带新计划/.test(拒发.error), '派发决定禁带新计划：' + 拒发.error);
    const r = S.表态(root, { 粒ID: g.粒ID, 预期版本: 1, 触发源: '今时线', 决定: '派发', 操作者: '总监' });
    assert.ok(r.ok, r.error);
    assert.equal(r.动作, '派发');
    assert.equal(r.单号, 'TK-500', '回单号给消费方走 lifecycle 放行');
    const now = S.取(root, g.粒ID);
    assert.equal(now.状态, '计划', '表态不代行 lifecycle：状态机一步不动');
    assert.equal(now.版本号, 1, '派发表态是审计痕，不顶版本');
    assert.ok(S.事件流(root).some((e) => e.事件类型 === '表态' && e.决定 === '派发' && e.触发源 === '今时线'), '派发表态留痕');
    S.转移(root, { 粒ID: g.粒ID, 目标: '起草中', 预期版本: 1, 操作者: '总监', 单号: 'TK-500' });
    const 晚 = S.表态(root, { 粒ID: g.粒ID, 预期版本: 2, 触发源: '今时线', 决定: '派发', 操作者: '项管' });
    assert.ok(!晚.ok && /计划态/.test(晚.error), '已过闸的粒谈不上派发：' + 晚.error);
  });

  await t('P2-3 表态-重排（H113）：类别∈五类+新计划+因缺一不可；估值变化须同事务带预估且新值≠现值', async () => {
    const root = makeRoot();
    const g = 登记一(root, { 预估单元: 2, 计划开始: '2026-08-20', 计划完成: '2026-08-22', 因: '排' });
    const 试 = (p) => S.表态(root, { 粒ID: g.粒ID, 预期版本: 1, 触发源: '今时线', 决定: '重排', 操作者: '项管', ...p });
    assert.match(试({ 新计划完成: '2026-09-01', 因: 'x' }).error, /必带类别/, '缺类别拒');
    assert.match(试({ 类别: '心情不好', 新计划完成: '2026-09-01', 因: 'x' }).error,
      /前置依赖未到\/额度不够\/并发满\/估值变化\/优先级不够/, '枚举外拒且拒因列全五类');
    assert.match(试({ 类别: '并发满', 新计划完成: '2026-09-01' }).error, /必须带因/, '缺因拒');
    assert.match(试({ 类别: '并发满', 因: 'x' }).error, /必带新计划/, '缺新计划拒（重排不给新排期等于没表态）');
    assert.match(试({ 类别: '估值变化', 新计划完成: '2026-09-01', 因: '估大了' }).error, /预估单元/, '估值变化不带预估拒');
    assert.match(试({ 类别: '估值变化', 新计划完成: '2026-09-01', 因: '估大了', 预估单元: 2 }).error, /不同于现值/, '预估没变不叫估值变化');
    assert.match(试({ 类别: '估值变化', 新计划完成: '2026-09-01', 因: 'x', 预估单元: -1 }).error, /须为正数/);
    assert.match(试({ 类别: '并发满', 新计划完成: '2026-09-01', 因: 'x', 预估单元: 5 }).error, /估值变化/, '非估值变化类不许顺手改预估');
    assert.match(试({ 类别: '并发满', 新计划完成: '2026-08-22', 因: '原地重排' }).error, /未改变任何排期字段/, '新计划=现计划不算重排');
    assert.equal(S.取(root, g.粒ID).版本号, 1, '以上全拒，现态一个字节没动');
    const r = 试({ 类别: '估值变化', 新计划完成: '2026-09-01', 因: '实测翻倍', 预估单元: 4 });
    assert.ok(r.ok, r.error);
    assert.equal(r.动作, '重排');
    assert.equal(r.推迟次数, 1);
    assert.ok(!r.升格, '第一次推迟不升格');
    const now = S.取(root, g.粒ID);
    assert.equal(now.计划完成, '2026-09-01');
    assert.equal(now.预估单元, 4, '预估与新计划同一事务落地');
    assert.equal(now.推迟次数, 1, '推迟次数进现态，闸与页面读得到');
    const e = S.事件流(root).find((x) => x.事件类型 === '重排');
    assert.ok(e && e.类别 === '估值变化' && e.触发源 === '今时线' && e.延期天 === 10,
      '重排事件带类别/触发源/延期天：' + JSON.stringify({ 类别: e.类别, 触发源: e.触发源, 延期天: e.延期天 }));
    assert.equal(e.字段变更.预估单元, 4, '同事务=同一条事件的字段变更');
    assert.equal(e.字段变更.推迟次数, 1);
    assert.equal(S.事件流(root).filter((x) => x.粒ID === g.粒ID && x.事件类型 !== '拒绝').length, 2,
      '登记+重排恰两条事件——估值不另开一条调整，中间断不了');
  });

  await t('P2-3 推迟累计：任何类别混着数，≥3 次返回 升格:true；终态不可表态重排', async () => {
    const root = makeRoot();
    const g = 登记一(root, { 计划完成: '2026-08-22', 因: '排' });
    const 挪 = (v, 完成, 类别) => S.表态(root, { 粒ID: g.粒ID, 预期版本: v, 触发源: '今时线', 决定: '重排', 类别, 新计划完成: 完成, 因: '挪', 操作者: '项管' });
    const a = 挪(1, '2026-08-25', '额度不够');
    assert.ok(a.ok && a.推迟次数 === 1 && !a.升格, '第 1 次不升：' + a.error);
    const b = 挪(2, '2026-08-28', '并发满');
    assert.ok(b.ok && b.推迟次数 === 2 && !b.升格, '第 2 次还不升');
    const c = 挪(3, '2026-08-31', '前置依赖未到');
    assert.ok(c.ok, c.error);
    assert.equal(c.推迟次数, 3);
    assert.equal(c.升格, true, '累计第 3 次升格总监——类别不同也一起数');
    const d = 挪(4, '2026-09-03', '优先级不够');
    assert.equal(d.升格, true, '≥3 持续举旗，不是只报一次');
    assert.equal(S.取(root, g.粒ID).推迟次数, 4, '推迟次数逐次累计进现态');
    assert.equal(c.延期天, 9, '延期照旧按基线累计（8-22 → 8-31）');
    const g2 = 登记一(root, { 题: '终态', 来源: 'z', 状态: '已成单', 单号: 'TK-601' });
    S.转移(root, { 粒ID: g2.粒ID, 目标: '完成', 预期版本: 1, 操作者: '项管' });
    assert.match(S.表态(root, { 粒ID: g2.粒ID, 预期版本: 2, 触发源: '今时线', 决定: '重排', 类别: '并发满', 新计划完成: '2026-09-09', 因: 'x', 操作者: '项管' }).error,
      /终态/, '终态待办不可表态重排');
  });

  await t('未排期粒（DS-12）：计划态且无计划开始才上榜；纯读不写盘', async () => {
    const root = makeRoot();
    assert.deepEqual(S.未排期粒(root), [], '空账回空表不炸');
    登记一(root, { 题: '没排', 来源: 'a' });
    登记一(root, { 题: '排了日', 来源: 'b', 计划开始: '2026-09-01', 因: 'x' });
    登记一(root, { 题: '排了刻钟', 来源: 'c', 计划开始: '2026-09-01T09:15', 计划完成: '2026-09-01T18:00', 因: 'x' });
    登记一(root, { 题: '只有完成日', 来源: 'd', 计划完成: '2026-09-05', 因: 'x' });
    const g5 = 登记一(root, { 题: '过了闸', 来源: 'e' });
    S.转移(root, { 粒ID: g5.粒ID, 目标: '起草中', 预期版本: 1, 操作者: '总监', 单号: 'TK-600' });
    const 账长 = fs.readFileSync(S.LOG(root), 'utf8').length;
    assert.deepEqual(S.未排期粒(root).map((x) => x.题).sort(), ['只有完成日', '没排'],
      '无计划开始=未排期（只有完成日也算——今时线扫的就是开始格）；两形态的已排期与非计划态都不上榜');
    assert.equal(fs.readFileSync(S.LOG(root), 'utf8').length, 账长, '纯函数：查询不写一个字节');
  });
