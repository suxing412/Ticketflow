...
// ── 弱特征补表（2026-08-26 TK-187/193/194 三例漏判）──
t('弱特征补表：监制台实体名 ≥2 词判疑似（TK-193 案），流程术语不误伤', () => {
  const 拦 = (文本) => 落点.查落点({ 项目: 'TK', 文本, cfg: {} });
  // TK-193 真形：只提实体名、不写路径
  const r1 = 拦('监制台待拍板队列积压约 49 条，制作人层逐条读完再拍板成本高，需分堆呈报');
  assert.equal(r1.ok, false, '监制台+待拍板队列 两词命中应判疑似');
  assert.equal(r1.疑似项目, 'Ticketflow');
  // 补的其余实体名同样成对生效
  assert.equal(落点.查落点({ 项目: 'TK', 文本: '瞭望塔账本水位与项管台账对不上', cfg: {} }).ok, false);
  // 不误伤①：单个弱词是日常（TK 单回执天天写「回执贴进监制台」）
  assert.equal(落点.查落点({ 项目: 'TK', 文本: '回执贴进监制台后再落袋', cfg: {} }).ok, true, '单词不作数');
  // 不误伤②：流程术语没进表——TK 单谈自己的在途/核查/盖印不该被拦
  assert.equal(落点.查落点({ 项目: 'TK', 文本: '本单在途期间盖印，核查过后落袋，回执贴进监制台', cfg: {} }).ok, true,
    '流程术语不算实体名，否则成片误杀');
});
