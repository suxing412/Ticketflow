...
t('拍板：想法→专项父单草稿（S 系列派号+回链）', () => {
  ...
  assert.equal(idea.状态, '已拍板'); assert.equal(idea.父单, 'TK-S1');
});
