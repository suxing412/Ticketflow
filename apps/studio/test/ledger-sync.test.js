...
(async () => {
  ...
  await t('新单挂粒：专项子单无粒 → 登粒（批=父单专项名 · 题=单题 · 单号挂链 · 依赖照 fm）', async () => {
    const root = makeRoot();
    铺专项(root);
    seed(root, '待审', { id: 'TK-156', title: '大纲树拖拽', 父单: 'TK-150', 职能: '程序', 依赖: ['TK-151', 'TK-152'] });
    const { 动作, 异常 } = 差(root);
  ...
  await t('不该登的不登：散单 / 非专项父单的子单 / 专项父单自身 / 已有粒的单', async () => {
    const root = makeRoot();
    铺专项(root);
    seed(root, '待审', { id: 'TK-200', title: '散单' });                                  // 无父单
    seed(root, '待审', { id: 'TK-201', title: '普通父', 父单类型: '普通' });
    seed(root, '待审', { id: 'TK-202', title: '普通父的子', 父单: 'TK-201' });              // 父单不是专项/战役
    seed(root, '待审', { id: 'TK-203', title: '父单不存在', 父单: 'TK-999' });
    seed(root, '待审', { id: 'TK-204', title: '嵌套专项', 父单: 'TK-150', 父单类型: '战役' }); // 自己也是容器
    ...
    seed(root, '待审', { id: 'TK-205', title: '已登甲', 父单: 'TK-150' });                  // 靠单号认
    seed(root, '待审', { id: 'TK-206', title: '已登乙', 父单: 'TK-150', 粒ID: g1.粒ID });   // 靠粒ID 认（单号对不上也认）
  ...
  await t('返修承袭：返工/推翻新号登粒时承袭原粒的批（不另起一个批）', async () => {
    ...
    seed(root, '归档', { id: 'TK-128', title: '水系描线', 父单: 'TK-150', 归档原因: '返工替代' });
    seed(root, '待审', { id: 'TK-170', title: '水系描线（推翻重做）', 父单: 'TK-150', 返工自: 'TK-128' });
    ...
    // 原粒无从查起时退回父单专项名
    seed(root, '待审', { id: 'TK-171', title: '孤返工', 父单: 'TK-150', 返工自: 'TK-888' });
    assert.equal(差(root).动作.find((x) => x.单号 === 'TK-171').批, '汉代地图专项');
  });

  // ---- ② 状态随单 ----
  await t('状态映射：十二态逐格（H108：待派算起草中 / 待处理·待重派算已成单 / 归档分岔 / 废弃撤销 / 挂起不动）', async () => {
    const 期 = {
      待审: '起草中', 待派: '起草中',                     // 派发钩子落在 待派→在途，这里不抢先成单
      待处理: '已成单', 待重派: '已成单',                 // 派发后的回流位：单发出去过，不退回起草中
      在途: '已成单', 初检: '已成单', 核查: '已成单', 仲裁: '已成单', // 审检链目录化仍是在流程中现身
      完成: '完成',                                       // 专项内部口径（H110）：子单到完成即算做完等关账
    };
    for (const s of store.STATES) {
      if (['归档', '废弃', '挂起'].includes(s)) continue;
      assert.equal(LS.目标状态({ state: s, fm: {} }), 期[s], `${s} 应映到 ${期[s]}`);
    }
    assert.equal(LS.目标状态({ state: '归档', fm: {} }), '完成', '无因归档=正常交付后的整理性归档（落袋口径）');
    for (const 因 of ['废弃', '验收打回', '定夺打回', '返工替代', '推翻替代（制作人翻案）']) {
      assert.equal(LS.目标状态({ state: '归档', fm: { 归档原因: 因 } }), '撤销',
        `带因归档（${因}）不算交付——历史「归档原因:废弃」留归档不改史，正靠这条仍判撤销`);
    }
    assert.equal(LS.目标状态({ state: '废弃', fm: {} }), '撤销', '废弃目录态=原「归档原因:废弃」独立化');
    assert.equal(LS.目标状态({ state: '挂起', fm: {} }), null, '挂起唯一可逆（→待重派）：粒不动，等复活或废弃再随单走');
  });

  await t('挂起随单不动（行为面）：粒已成单 + 单进挂起 → 差量零动作，粒停在已成单', async () => {
    const root = makeRoot();
    铺专项(root);
    const g = S.登记(root, [{ 批: '编辑器专项', 序: 1, 题: '被挂单', 来源: 's', 状态: '已成单', 单号: 'TK-180' }], '总监').新增[0];
    seed(root, '挂起', { id: 'TK-180', title: '被挂单', 父单: 'TK-150', 挂起: { 操作者: '制作人', 理由: '等实拍' } });
    assert.deepEqual(差(root).动作, [], '挂起既不收成完成也不撤销——两种都是提前定论');
    LS.同步(root, { 触发: '例行' });
    assert.equal(S.取(root, g.粒ID).状态, '已成单', '同步跑完粒原地不动');
  });
  ...
  await t('只前进不倒退：粒已终态不动；H65 返修同号回待审不把已成单粒拽回去', async () => {
    const root = makeRoot();
    铺专项(root);
    const g = S.登记(root, [{ 批: '编辑器专项', 序: 1, 题: '返修单', 来源: 's', 状态: '已成单', 单号: 'TK-167' }], '总监').新增[0];
    seed(root, '待审', { id: 'TK-167', title: '返修单', 父单: 'TK-150', 粒ID: g.粒ID, 返修轮: 1 }); // H65 返修：单回待审（原 草稿），号不变
  ...
  await t('扫事件：journal 增量按关键词命中；自产行不自触发；光标只推到完整行末', async () => {
    ...
    assert.ok(首.光标.偏移 > 0);
    写('[2026-08-12 21:01] 派发 TK-156（待派→在途）\n[2026-08-12 21:02] 保存：TK-9 无实质变更\n');
  ...
  await t('去抖合并：连发三条事件只落一次同步，锚点钉在第一条上', async () => {
    ...
    seed(root, '待审', { id: 'TK-158', title: '丙', 父单: 'TK-150' });
  ...
  await t('首跑全量回填：编辑器专项 11 单 5 粒 → 补齐 156~161 六粒', async () => {
    ...
    // 忘登的 6 张（156~161），状态各异（H108 十二态铺样：含审检链目录态）
    const 忘登 = [['TK-156', '属性面板', '完成'], ['TK-157', '预览联动', '完成'], ['TK-158', '快捷键表', '归档'],
      ['TK-159', '多选框选', '在途'], ['TK-160', '导出校验', '核查'], ['TK-161', '空态文案', '待审']];
