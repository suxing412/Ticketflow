...
// H49「拉取制退役、派发制立宪」的发行侧判据（2026-08-19 对账审计：立宪没落到发行默认值）。
// runner 读 cfg.执行器.派发制，**缺键即 false** → 模板不写这一格，新部署开机就跑已废止的拉取制：
// 单子放行了在待投干等，没有常驻 agent 会来领。这条回归=每台新机重演一次「放行了却没人干活」。
t('模板钉死派发制（缺键即回落已废止的拉取制——立宪不该靠记得手填）', () => {
  assert.equal(setup.模板配置().执行器.派发制, true);
});

t('向导落地的配置读出来就是派发制（新部署开箱即走现行制度）', () => {
  const d = 新目录();
  setup.建工作区(d);
  const cfg = require('../lib/core/config').load(d);
  assert.equal(!!(cfg.执行器 && cfg.执行器.派发制), true, '新部署默认必须是派发制');
});

t('两份模板不许分叉：套件模板与内置模板的派发制口径一致（源码布局才校，打包后无套件）', () => {
  const p = path.resolve(__dirname, '..', '..', '..', '套件', 'studio.config.template.json');
  if (!fs.existsSync(p)) return; // 打包产物里没有套件/，跳过不算失败
  const 套 = JSON.parse(fs.readFileSync(p, 'utf8').replace(/^﻿/, ''));
  assert.equal(!!(套.执行器 && 套.执行器.派发制), setup.模板配置().执行器.派发制,
    '套件模板与内置模板对同一格给出不同答案，就是下一次「新部署行为不一致」的病根');
});
