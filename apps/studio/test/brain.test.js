...
t('切单提示词带归属两处：口径条目 + 输出契约字段清单（H107 换轴）', () => {
  // **换轴（2026-08-23）**：原样断言的是 H51 挂载律的措辞「管线归属必填…必须写 管线: P-N」。
  // 制作人当日裁定「四层架构应该写历史决议，新决议覆盖旧的」——H107 取代 H51 挂载律，
  // 归属改为**只记直接上级**（工单记专项，管线由 专项→特性→管线 推导）。
  // 这一格随之换轴：它要守的从来不是「管线两个字在不在」，而是「口径条目 + 输出契约两处都在」。
  const { buildCutPrompt } = require('../lib/pm/brain');
  const root = makeRoot();
  const p = buildCutPrompt(root, {}, { id: 'TK-S9', fm: { 项目: 'SLG' }, body: '造地图' }, '');
  assert.ok(/只记直接上级/.test(p), '口径条目在');
  assert.ok(/专项: <本单所属专项号/.test(p), '输出契约字段清单在——不列字段等于不让写');
  assert.ok(!/管线归属必填/.test(p), 'H51 时代的措辞不许留着——留着就是教 agent 多记一遍');
  assert.ok(!/'管线: <本单所属管线号/.test(p), '契约里不许再列 管线 格');
...
/* ===================== 三大态改口（H78 第 4 站 · 2026-08-24）=====================
 * 提示词是发给 agent 的行为面：里面写旧态词，agent 就照旧态办事。定式沿用本套件既有做法——
 * 真调 buildCutPrompt/buildDraftPrompt，断言含新词不含旧词。
 * 注意保留词：「在途」「完成」新旧同名，不许断言不含（合法出现）；「池」是别词的子串（执行池），不设含断言。 */
t('切单提示词已随三大态改口：新词在、旧态词零命中', () => {
  const { buildCutPrompt } = require('../lib/pm/brain');
  const root = makeRoot();
  const p = buildCutPrompt(root, {}, { id: 'TK-S9', fm: { 项目: 'SLG' }, body: '造地图' }, '');
  for (const 新 of ['待审', '初检', '待处理']) assert.ok(p.includes(新), `切单提示词该有新态词「${新}」`);
  for (const 旧 of ['草稿', '待投', '质检', '待验收', '待定夺', '已归档', '执行失败']) {
    assert.ok(!p.includes(旧), `切单提示词残留旧态词「${旧}」——H78 第 4 站没走完`);
  }
});

t('起草提示词已随三大态改口：旧态词零命中（含 FIELD_RULES 注入段）', () => {
  const { buildDraftPrompt } = require('../lib/pm/brain');
  const p = buildDraftPrompt({}, '把海岸线钉零', 'D:/proj', '');
  for (const 旧 of ['草稿', '待投', '质检', '待验收', '待定夺', '已归档', '执行失败']) {
    assert.ok(!p.includes(旧), `起草提示词残留旧态词「${旧}」`);
  }
  assert.ok(/=== 制作人层需求 ===[\s\S]*把海岸线钉零/.test(p), '改口没挤掉需求段');
});
...
t('取数层：完结单的「预计 vs 实际」两端凑齐（时间取工单戳、token 取预算账，可注入）', () => {
  ...
  seed(root, '归档', { id: 'TK-201', 职能: '程序', 单型: '实现单', 预计时间: '0.5', 预计token: '50000',
    领单时间: '2026-08-10T00:00:00.000Z', 交付时间: '2026-08-10T01:00:00.000Z' });
  seed(root, '在途', { id: 'TK-202', 职能: '程序', 单型: '实现单', 预计时间: '0.5' }); // 没完结：不该进样本
  seed(root, '废弃', { id: 'TK-203', 职能: '程序', 单型: '实现单', 预计时间: '0.5',   // 三大态改造：废弃=没做完，
    领单时间: '2026-08-10T00:00:00.000Z', 交付时间: '2026-08-10T01:00:00.000Z' });     // 戳齐全也不进样本
  ...
  assert.deepEqual(单表.map((x) => x.id).sort(), ['TK-200', 'TK-201'], '只取 完成/归档（三大态改造：完成=做完候验收也算样本；在途/废弃不进）');
