...
t('② 聚焦投影：聚焦 S-1 可见行集合=祖先链+子孙（集合相等）；面包屑在；折叠支里的叶子祖先链强制展开；退出全量恢复', () => {
  ...
  // 今时线（08-25 制作人「画得好看一点」案）：徽章文字色定死深色实色（--card 在 glass 近全透明，
  // 「橙块没字」病＝透明变量病第二案）；今时线全族 z:1 让位 sticky 树列（原 z:7/3 横滚压树列字）。
  const 今块 = (css25.match(/\.gt2now\.h em \{[^}]+\}/s) || [''])[0];
  assert.ok(今块.includes('color:#1f1509'), '今徽文字色须定死深色实色——主题变量在 glass 下透明（橙块没字病）');
  assert.ok(!/\.gt2now[^{]*\{[^}]*z-index:(?:[2-9]|\d\d)/s.test(css25),
    '今时线族 z 必须 <2——高 z 横滚时会压 sticky 树列的字');
