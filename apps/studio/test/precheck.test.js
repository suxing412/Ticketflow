...
// ============ 判定符前置 + 前缀词形态（2026-08-26 制作人 12:37 抓的候人裁积压案）============
// 案源：TK-197 回执写 `- ✓ 标准 1（…）：…`，八条应答齐全却判 0/8——旧法只认「N.」「第 N 条」
// 「表格」「圈号」四形，判定符写在编号前、编号前缀词是「标准」时一条都认不出。
// 同报错卡住 TK-185/190/203/204/TF-4 一批单在「候人裁」，属**形式性误拦**（判据窄，不是回执坏）。
// 边界：判定符前置时必须带前缀词（标准/验收/条目/第/项/AC/#）才认——自由散文里的数字不许冒充应答。
const 前置标准表 = 标准表('编译零错误', '新增测试在位', '全量测试全绿');

// 假绿纠正（2026-08-26 总监自查）：下面两条初版只断言端到端「过」，而它们过关靠的是
// run() 的宽松放行分支（应答表 0 条 + 判定符足额），并非形态被识别——判据必须直接断言
// 应答表 的产物，否则「解析器认不认得这个形态」这件事根本没被测到。
await t('应答表·围栏内的伪编号不算应答（TK-197 致命第二层）', () => {
  const 自测 = `- ✓ 标准 1：检索证据
...
- ✓ 标准 2：在位`;
  const r = precheck.应答表(自测);
  assert.deepEqual([...r.map.keys()], [], '围栏内的 `5.` / `| 3 |` 一个都不许进应答表');
  assert.ok(r.判定符数 >= 2, '围栏外的判定符照数：' + r.判定符数);
  // 致命性自证：只要污进一条，应答表非空 → 绕过宽松放行 → 逐条比对把齐全回执打成「已答 1 条」
});

await t('形态·判定符前置 + 「标准 N」前缀（TK-197 案）：三条全认，不再 0/3', () => {
  const root = makeRoot();
  const 回执 = `# 完工报告 X-97

## 自测结果

- ✓ 标准 1（编译面）：dotnet build 退出码 0
- ✓ 标准 2：Foo.cs 在位
- ✗ 标准 3：2 例红，见异议
...
## 异议
无`;
  const t1 = 铺(root, 'X-97', 回执, { body: 前置标准表 });
  const r = precheck.run(root, t1, CFG);
  assert.equal(r.统计.标准条数, 3);
  assert.equal(r.统计.应答条数, 3, '判定符前置 + 「标准 N」三条全认（人工判读同数）：' + JSON.stringify(r.缺项));
  assert.ok(!r.缺项.some((x) => x.includes('逐条') || x.includes('未逐条应答')), '不该再报逐条缺项：' + JSON.stringify(r.缺项));
});

await t('形态·前缀词族与判定符后置并存：验收/条目/AC/第 N 条 都认；散文数字不冒充应答', () => {
  const root = makeRoot();
  const 回执 = `# 完工报告 X-98
...
- ✔ 条目 2 —— 断言在位
- ✓ AC 3：零回归
- 说明：本轮改了 3 处文件，跑了 2 次全量，均与验收条目无关

## 实际消耗
约 20 分钟；约 50000 tokens

## 异议
无`;
  const t1 = 铺(root, 'X-98', 回执, { body: 前置标准表 });
  const r = precheck.run(root, t1, CFG);
  assert.equal(r.统计.应答条数, 3, '三种前缀词全认：' + JSON.stringify(r.缺项));
  assert.equal(r.初检, '过', JSON.stringify(r.缺项));
  // 反向：散文行「改了 3 处文件」不带判定符也不带前缀词，不许被数成第 3 条应答（数已是 3，
  // 若误认会变成重复覆盖——用一份只有散文的回执反证）
  const root2 = makeRoot();
  const 散文回执 = `# 完工报告 X-99

## 自测结果

跑了 3 次全量，改了 2 处文件，耗时 1 小时——全部通过。
...
## 异议
无`;
  const t2 = 铺(root2, 'X-99', 散文回执, { body: 前置标准表 });
  const r2 = precheck.run(root2, t2, CFG);
  assert.equal(r2.统计.应答条数, 0, '散文里的数字不许冒充逐条应答（误认比误拦更危险）：' + JSON.stringify(r2.统计));
});

await t('围栏内 shell 注释不腰斩章节（TK-197 真凶）：代码块里的 `# stdout:` 不是标题', () => {
  const root = makeRoot();
  const 回执 = `# 完工报告 X-96
...
## 实际消耗
约 30 分钟；约 60000 tokens

## 异议
无`;
  const t1 = 铺(root, 'X-96', 回执, { body: 前置标准表 });
  const 自 = precheck.章节(precheck.末轮(回执), '自测结果');
  assert.ok(自 && 自.includes('标准 3'), '自测节必须完整跨过代码块——围栏内的 `# stdout:` 不是章节标题');
  const r = precheck.run(root, t1, CFG);
  assert.equal(r.统计.应答条数, 3, '腰斩则只剩标准 1，实得：' + JSON.stringify(r.统计));
  assert.equal(r.初检, '过', JSON.stringify(r.缺项));
});

await t('围栏内的伪轮标不切轮（同族）：代码块里的「# 完工报告」不算新一轮', () => {
  const 回执 = `# 完工报告 X-95
...
## 实际消耗
约 10 分钟；约 20000 tokens

## 异议
无`;
  assert.equal(precheck.轮段(回执).length, 1, '围栏内的轮标不许切轮——切了就把末轮判在半截代码上');
});

// #71 余量：本套 6 例走 runner.tick（内含 pool.claim / gates.canPull 那条路）。
// 顶部的 quota 打桩光有桩不算判据——桩子被删掉测试照绿。这一格把「有没有真外呼」变成账。
// 判官段不算执行方的字（2026-08-28 TF-15 案）。
//
// 回执是分层追加的：执行方写完，质检/核查/仲裁/初检各自往后追加自己的段。
// 本日给质检报告补上「入回执」（此前全仓 196 份回执含质检报告 0 份）之后，
// TF-15 当场被判「回执含空壳标志：In progress×1、Waiting×1」——那两个词出自**质检报告**，
// 执行方的回执里一个都没有。修一处漏，撞出另一处的隐含假设：
// 禁语这条判据一直默认「回执里只有执行方的字」，而那个前提刚刚不成立了。
await t('禁语只扫执行方那部分：判官追加的段落里出现禁语，不算执行方交空壳', () => {
  const P = precheck.参数({});
  const 回执 = ['# 完工报告 X-1', '', '## 自测结果', '1. ✓ 判据一：实跑绿', '',
    '## 实际消耗', '约 10 分钟；约 4800 tokens', '', '## 异议', '无', '',
    '## 质检（通过）', '| 项 | 状态 |', '| 1 | In progress |', '| 2 | Waiting |', '',
    '## 两检初检（机判）', '结论：不过'].join('\n');
  assert.ok(/In progress/.test(回执) && /Waiting/.test(回执), '夹具本身要真含禁语，否则这条判据什么都没验');
  assert.ok(!/In progress|Waiting/.test(precheck.剥判官段(回执)), '剥掉判官段后不该再有禁语');
  assert.match(precheck.剥判官段(回执), /判据一：实跑绿/, '执行方自己的字一个不许丢');
  void P;
});

// 上一条只验了 剥判官段 这个函数本身——**没验它真的接在 run() 上**。
// 实测：把 run() 里那一处调用改回 `本轮.match(re)`，上一条照样绿。
// 一个只测零件不测接线的判据，等于没测（今夜第二次踩同一形状：G27 的两处防护也互为冗余）。
await t('端到端：run() 真跑一遍——判官段里的禁语不该把执行方判成空壳', () => {
  const root = makeRoot();
  const 回执 = 好回执 + ['', '', '## 质检（通过）', '| 项 | 状态 |', '|---|---|',
    '| 1 | In progress |', '| 2 | Waiting |', '', '## 两检初检（机判）', '结论：不过'].join('\n');
  assert.ok(/In progress/.test(回执), '夹具要真含禁语');
  const t1 = 铺(root, 'X-70', 回执);
  const r = precheck.run(root, t1, CFG);
  assert.equal(r.初检, '过', '判官措辞被算到执行方头上了：' + JSON.stringify(r.缺项));
  assert.deepEqual(r.统计.禁语命中, [], '禁语命中应为空——那两个词不在执行方写的那部分里');
});

await t('反向：执行方自己写了禁语，照旧判不过（别把闸一起剥掉）', () => {
  const root = makeRoot();
  const 回执 = 好回执.replace('## 异议\n无', '## 异议\n本地跑批 In progress。')
    + ['', '', '## 质检（通过）', '判官写的正常文字'].join('\n');
  const t1 = 铺(root, 'X-71', 回执);
  const r = precheck.run(root, t1, CFG);
  assert.equal(r.初检, '不过', '执行方自己的空壳标志必须照抓');
  assert.ok(r.缺项.some((x) => x.includes('In progress')), JSON.stringify(r.缺项));
});

// 首版只剥掉了判官报告的**第一段**：它问的是「这一行是不是判官标题」，
// 遇到不认识的标题就恢复保留——而真实质检报告自己带一堆同为 ## 级的子标题
// （`## 第〇步 产出存在性核验`、`## 核验局限`、`## 结论`…），后面照旧漏进禁语扫描。
// 判别方向反了：判官子标题穷举不完，执行方章节名却是个小的已知集合。
await t('判官报告自带 ## 级子标题时，整份都要摘干净（首版只摘了第一段）', () => {
  const 文 = ['## 自测结果', '1. ✓ 过', '',
    '## 质检（通过）', '逐条核验如下', '',
    '## 第〇步 产出存在性核验', '| 1 | In progress |', '',
    '## 核验局限', 'Waiting 中', '',
    '## 异议', '执行方的异议'].join('\n');
  const 剥 = precheck.剥判官段(文);
  assert.ok(!/In progress|Waiting/.test(剥), '判官报告的子节漏网了：\n' + 剥);
  assert.match(剥, /1\. ✓ 过/, '执行方的自测结果要留');
  assert.match(剥, /执行方的异议/, '判官段之后执行方的章节要恢复保留');
});

await t('剥判官段只摘判官那几节：执行方的普通章节照旧保留', () => {
  const 文 = ['## 做了什么', '改了三个文件', '## 核查', '判官写的', '## 异议', '执行方写的异议'].join('\n');
  const 剥 = precheck.剥判官段(文);
  assert.match(剥, /改了三个文件/);
  assert.match(剥, /执行方写的异议/, '判官段之后的执行方章节要能恢复保留——摘到文末就把它一起吃了');
  assert.ok(!/判官写的/.test(剥));
});
