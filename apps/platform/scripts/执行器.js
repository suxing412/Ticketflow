...
const 服务 = http.createServer((req, res) => {
  ...
  if (m && req.method === 'POST') {
    ...
    return 收体(req, 256 * 1024, (体) => {
      ...
      记开跑(id, '执行', 派.选中);                      // 停机时要给它盖章（协-019）；写态让别的进程看得见它在跑
      (async () => {
      ...
      })().finally(() => 记跑完(id, '执行'));   // 跑完（成或败）就出清单，别让停机去盖一张已经结束的单
    });
  }
...
function 写态(因) {
  try {
    ...
    fs.writeFileSync(p, JSON.stringify({
      ...
      // 已跑毫秒 现算——读这份文件的人关心的是「跑了多久了」，不是「什么时候起的」。
      在跑: [...在跑单].map(([单, v]) => ({ 单, ...v, 已跑毫秒: Date.now() - Date.parse(v.起于) })),
    }, null, 2) + '\n', 'utf8');
