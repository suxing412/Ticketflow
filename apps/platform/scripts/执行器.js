...
const 服务 = http.createServer((req, res) => {
  ...
  if (路径 === '/tick') {
    ...
    const 现在 = Date.now();
    const 有效挂起 = (t) => {
      const 挂 = 工单库.挂起状态(t.fm, 现在);
      return 挂.挂起 && !挂.已到期;
    };
    // 挂起单在此拍显式单列到回执，不再作为可派候选或占用并发的在途单。
    const 待投表 = 全部.filter((t) => t.state === '待投' && !有效挂起(t))
      .map((t) => 工单库.find(工单根.根, t.id)).filter(Boolean);
    // 卡死阈值复用巡检那把尺（默认 30 分钟）。两处用不同阈值的话，
    // 会出现「巡检说它超时了、调度说它正常在跑」——同一台机器对同一张单两种说法。
    const 卡死阈值 = Number((配置.巡检 && 配置.巡检.在途超时毫秒) || 30 * 60 * 1000);
    const { 在跑, 疑似卡死 } = 调度.统计在跑(全部.filter((t) => t.state === '在途' && !有效挂起(t)), 现在, 卡死阈值);
