...
function 退出处置(条, 码, 信号) {
  ...
  条.崩史.push(现在);
  const 次 = 条.崩史.length;
  // 端口冲突的那句话由子进程自己说（lib/实例身份.js 会去探对方的健康检查，
  // 报出对方的 pid 与配置目录）。这里只做转述与兜底——原先这句是泛泛的
  // 「多半是上一次没关干净，或者你另开了一份」，两种猜测都不落地，
  // 而真实那次的答案是「另一份 platform，用着另一个 PLATFORM_CONFIG」。
  const 尾 = 条.端口冲突
    ? `${条.口} 端口已被占用（上面 [${条.名}] 那几行说了对方是谁）。`
      + `同机两份 platform 会抢同一组端口——PLATFORM_CONFIG 只换配置目录、不换端口。`
      + `还认不出来就查：netstat -ano | findstr :${条.口}`
    : '';
...
process.stdout.write(
  ...
  // 用了非默认配置目录就得说破（协-038）：它换的只是配置目录，端口照旧，
  // 所以「换个 PLATFORM_CONFIG 就能并行跑一份」是个错觉——两份必抢同三个端口。
  + (() => {
    const 横幅 = require(path.join(平台根, 'lib', '实例身份.js')).横幅(平台根, 端口表);
    return 横幅 ? `[开机] ${横幅}\n` : '';
  })());
