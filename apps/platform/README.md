...
**新装一台机器：双击 `部署.bat`**（非 Windows：`node scripts/首装.js` 然后 `npm start`）。
它验 node、问答式写好那几个 `config/*.local.json`、然后打一遍自检。
**验收标准只有一条：自检显示「全链路就绪」。** 前置要求与逐项说明见 [SETUP.md](SETUP.md)。

首装可以反复跑：已配好的一律保留并说明跳过了什么，升级或补配都直接重跑。
非交互装机（CI、批量铺）：`node scripts/首装.js --yes --工单库 "..." --项目名 X --项目路径 "..."`。
...
> `/api/health` 还**自报身份**：`pid` / `启动于` / `配置目录` / `配置来源`。
> 同机跑过两份 platform 才补的：`PLATFORM_CONFIG` 换的只是**配置目录**，端口仍从
> `platform.config.json` 读，两份必抢同一组 4370/4371/4372。当时 4370 上答话的是另一份，
> 而 `health` 照答 `ok: true`——最坏的一类假信号。撞端口时进程也会先探一下对方的
> `health`，直接印出「谁占着、配置目录是哪个、pid 多少」，不必再 `netstat` + 读进程命令行。
> 另外三个进程各自盯着监工（IPC 断开即自停）：监工先没了、server 独活占着端口的空壳，
> 这条负责收掉。
