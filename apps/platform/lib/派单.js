...
// 只读产出 —— 这张单的交付物**本来就不是文件**（协-020）。
//
// 空转闸的前提是「跑成功就该有改动」，那对写代码的单成立，对判定类的单不成立：
// reviewer 交的是「结论：通过/不过 + 依据」，工单模板里它的验收标准就是那一行。
// 把这种单按空转退回待投，等于**永远做不完**——它跑一百次都不会有文件改动。
//
// 判据只认**显式声明**，不靠角色猜：声明了 write_scope 就是打算写（一票否决），
// 否则看产出物类型是不是那几个不落盘的。宁可漏判成普通单（顶多退回待投让人看一眼），
// 也不能把一个真该写代码却什么都没写的单说成「它本来就不用写」——那是把失败洗成成功。
...
module.exports = { 权限参数, 冻结情况, 账本体检, 真跑前提, 选派, 落单, 依赖就绪, 写权矛盾, 受限即只读, 只读产出 };
