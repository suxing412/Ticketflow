...
t('权限：白名单之外一律受限，缺配置即最严不是最松', () => {
  ...
  // 受限**参数**按适配器分（协-004 改）：不知道是哪家 CLI 就注不进任何 flag。
  // 这比原先「一律注 claude 那套」更准确——把 claude 的 flag 塞给 codex 会让判官每次必挂。
  assert.deepEqual(派单.权限参数({}, 'x', 'claude-cli').参数, ['--permission-mode', 'plan']);
  assert.deepEqual(派单.权限参数({}, 'x').参数, [], '不知道适配器就注不进参数');
  assert.ok(/形同虚设/.test(派单.权限参数({}, 'x', '未知-cli').警告 || ''),
    '注不进参数等于没受限，必须明说而不是静默放行');
  // 空白名单 → 全部受限
...
// ---- 闸③后半：吃满没有 ----
t('预算闸读不到数时不许真跑（「什么都不冻」跟「都没超」长得一样）', () => {
  // 闸③其实是两半：「配没配上限」读配置，稳；「超没超」要载 budget 公用件算，会失效。
  // 而失效时 冻结情况 返回 挡:{}——**一个池都不冻**，和「都没超」是同一个形状。
  // 于是闸门静默消失，人看到的仍是一切正常，实际用量已经没人管了。
  const 坏公用件 = { 载入: () => { throw new Error('公用件加载失败 budget/budget.js'); } };
  const 冻 = 派单.冻结情况(坏公用件, { 预算: { 池: { claude: { 日token: 100 } } } }, 平台根);
  assert.deepEqual(冻.挡, {}, '这就是那个陷阱：光看「挡」分不出是没超还是没读到');
  assert.equal(冻.ok, false, 'ok 是唯一能分辨的字段——调用方必须看它');
  const 前提 = 派单.真跑前提(冻);
  assert.equal(前提.准, false, '读不到用量还放行，等于闸没了');
  assert.equal(前提.码, 503);
  assert.ok(/干跑不受影响/.test(前提.错), '停的只该是真跑，别把演练也堵死：' + 前提.错);
  // 正常读到数就该放行，别把闸做成一停到底
...
t('本地覆盖：深合并而非整段替换，且只认白名单文件', () => {
  ...
  // 跟机器走不跟代码走，入库那份只能是空的；而空的含义是「未声明」＝按会计费对待。
    // routing 于 2026-08-13（协-015 编制）加入：「哪个角色归哪个模型」跟**这台机器上有哪些池**
  // 有关，别人 clone 走这个仓，他的池未必叫同样的名字，入库一份指名道姓的偏好表
  // 只会让他的第一次派活出错。
  assert.deepEqual(Object.values(覆盖.覆盖表).sort(), ['workspace', '执行', '预算', '项目', '计费', 'routing'].sort());
