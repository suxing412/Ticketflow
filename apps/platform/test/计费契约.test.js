...
// ---- 自检的口径（协-008 漏了这一处，协-009 补）----
// 自检是**产品对自己的描述**，也是首次使用最先看到的东西。
// 口径不一致比措辞难看严重：人照着它判断要不要点下去。
// 它此前一条测试都没有，而 2026-08-12 就是在它的输出上看到「真跑（会花钱）」——
// 别处全改完了，唯独这里还挂着旧说法。
const 自检 = require(path.join(平台根, 'lib', '自检.js'));

t('自检不把「真跑」直接说成「花钱」', () => {
  const 条 = 自检.查(平台根, { providers: { claude: {} }, 计费: { claude: { 模式: '订阅' } } }, { ok: true, 根: 'x' });
  const 真跑 = 条.find((x) => x.能力.startsWith('真跑'));
  assert.ok(真跑, '自检里该有「真跑」这条能力');
  assert.ok(!/花钱/.test(真跑.能力), '能力名里还写着「花钱」：' + 真跑.能力);
});

t('自检要报出每个池的计费方式——那正是点「真跑」之前要知道的事', () => {
  const 条 = 自检.查(平台根, {
    providers: { claude: {}, echo: {}, gpt: {} },
    计费: { claude: { 模式: '订阅', 订阅名: 'Claude Pro' }, echo: { 模式: '本地' } },
    执行: { 允许真跑: true }, 预算: { 池: { claude: { 日token: 1 } } },
  }, { ok: true, 根: 'x' });
  const 真跑 = 条.find((x) => x.能力.startsWith('真跑'));
  assert.equal(真跑.现值.计费.claude, '订阅额度');
  assert.equal(真跑.现值.计费.echo, '本地执行');
  assert.equal(真跑.现值.计费.gpt, '计费方式未声明');
  // 未声明的池要点名。默认按会计费对待，但人得知道是哪几个，才知道去哪儿补声明。
  assert.ok(/gpt/.test(真跑.现值.提醒 || ''), '未声明的池要点名：' + JSON.stringify(真跑.现值));
});

t('自检在参数不全时也要出表，不能抛', () => {
  // 它是**出事之后才会被跑**的东西。在环境不全时自己先崩掉的诊断工具，
  // 正好在最需要它的时刻掉链子。
  for (const 参 of [[平台根], [平台根, null], [平台根, {}, null], [平台根, null, null]]) {
    assert.doesNotThrow(() => {
      const 条 = 自检.查(...参);
      assert.ok(Array.isArray(条) && 条.length, '出的表不能是空的');
      assert.ok(自检.结论(条).级别, '结论要给得出级别');
    }, '参数 ' + JSON.stringify(参.slice(1)) + ' 时抛了');
  }
});
