...
t('巡检：有效挂起单单列「挂起中」，不进逾期、零派发、依赖或三振告警', () => {
  const 全 = [
    { id: 'S-在途', state: '在途', fm: { 派单时间: '2026-08-10T10:00:00.000Z', 挂起: { 状态: '挂起', 原因: '等制作人裁决', 开始时间: '2026-08-10T10:30:00.000Z' } } },
    { id: 'S-待投', state: '待投', fm: { 挂起: { 状态: '挂起', 原因: '等外部资料', 开始时间: '2026-08-10T10:31:00.000Z' }, 依赖: ['幽灵'] } },
  ];
  const 战绩 = [1, 2, 3].map(() => ({ role: 'reviewer', qualityPassed: false, ticket: 'S-在途' }));
  const 告 = 巡检.巡一轮({}, { 全部工单: 全, 现在, 冻结: {}, 本轮派出: 0, 本轮跳过: [], 战绩 });
  assert.equal(告.length, 0, '挂起单不应产生任何告警');
  const 节 = 巡检.挂起中(全, 现在);
  assert.deepEqual(节.map((x) => x.单).sort(), ['S-在途', 'S-待投']);
  assert.equal(节.find((x) => x.单 === 'S-在途').原因, '等制作人裁决');
  assert.equal(节.find((x) => x.单 === 'S-待投').起始时间, '2026-08-10T10:31:00.000Z');
});

t('巡检：A 依赖挂起中 B，不产生依赖缺失告警', () => {
  const 全 = [
    { id: 'A', state: '待投', fm: { 依赖: ['B'] } },
    { id: 'B', state: '待投', fm: { 挂起: { 状态: '挂起', 原因: '等待外部审批', 开始时间: '2026-08-10T10:30:00.000Z' } } },
  ];
  const 告 = 巡检.巡一轮({}, { 全部工单: 全, 现在, 冻结: {}, 本轮派出: 1, 本轮跳过: [], 战绩: [] });
  assert.equal(告.some((x) => x.单 === 'A' && x.类型 === '依赖缺失'), false,
    '挂起的上游仍在工单全集中，不能被误报为缺失');
  assert.deepEqual(巡检.挂起中(全, 现在).map((x) => x.单), ['B']);
});

t('巡检：挂起到期后恢复正常计时并照常告警，不再留在挂起中节', () => {
  const 全 = [{
    id: 'S-到期', state: '在途', fm: {
      派单时间: '2026-08-10T10:00:00.000Z',
      挂起: { 状态: '挂起', 原因: '短暂停止', 开始时间: '2026-08-10T10:10:00.000Z', 到期时间: '2026-08-10T11:00:00.000Z' },
    },
  }];
  const 告 = 巡检.巡一轮({}, { 全部工单: 全, 现在, 冻结: {}, 本轮派出: 0, 本轮跳过: [], 战绩: [] });
  assert.ok(告.some((x) => x.单 === 'S-到期' && x.类型 === '在途超时'));
  assert.equal(巡检.挂起中(全, 现在).length, 0);
});

t('巡检：挂起字段有到期时间时，时间尚未到不触发逾期告警', () => {
  const 全 = [{
    id: 'S-未到期', state: '在途', fm: {
      派单时间: '2026-08-10T10:00:00.000Z',
      挂起: { 状态: '挂起', 原因: '等供应商', 开始时间: '2026-08-10T10:30:00.000Z', 到期时间: '2026-08-10T13:00:00.000Z' },
    },
  }];
  const 告 = 巡检.巡一轮({}, { 全部工单: 全, 现在, 冻结: {}, 本轮派出: 0, 本轮跳过: [], 战绩: [] });
  assert.equal(告.length, 0);
  assert.equal(巡检.挂起中(全, 现在)[0].到期时间, '2026-08-10T13:00:00.000Z');
});
