...
t('运行态在**每次进出在跑清单**时都落盘（协-021）', () => {
  ...
  // 协-027 把 add/delete 收进了 记开跑/记跑完 两个小函数——**写态仍必须贴着它们**，
  // 这条断言守的是那个贴着的关系，不是某一种写法。
  const 源 = fs.readFileSync(path.join(平台根, 'scripts', '执行器.js'), 'utf8');
  assert.match(源, /在跑单\.set\([^;]*\);\s*写态\(/, '加进在跑清单时要写一次态，否则别的进程看不见它在跑');
  assert.match(源, /在跑单\.delete\([^;]*\);\s*写态\(/, '跑完出清单也要写一次，否则态文件会一直挂着一张早跑完的单');
});

t('进出清单的时机必须贴着 spawn 与回调，不能挂在 .finally 上（协-027 实测）', () => {
  // 原先 记跑完 挂在 async IIFE 的 .finally()，而 拉起 是**回调式**的——
  // IIFE 一调用它就 resolve，finally 当场触发，而 CLI 还在后台跑。
  // 实测：一次 183 秒的真跑，开跑后 22 秒态文件就写了「跑完」，在跑全程是空的。
  // 协-021 当时只断言了源码模式、没有实测，所以这个洞一直在——**这条只能靠位置守**。
  const 源 = fs.readFileSync(path.join(平台根, 'scripts', '执行器.js'), 'utf8');
  assert.ok(!/\.finally\(\(\) => 记跑完/.test(源), '别再挂回 finally——那是假的完成信号');
  for (const 类 of ['执行', '质检']) {
    const 开 = 源.indexOf(`记开跑(id, '${类}'`);
    const 完 = 源.indexOf(`记跑完(id, '${类}'`);
    const 拉 = 源.indexOf('拉起(', 开);
    assert.ok(开 > 0 && 完 > 0, `${类}：两头都要有`);
    assert.ok(开 < 拉 && 拉 < 完, `${类}：开跑要贴在 拉起 之前、跑完贴在回调第一行`);
  }
});

t('未进主线要进欠账清单，不能只在看板上挂个徽标（协-025）', () => {
  // HW-4 实测：活干完了、检查点落了、看板上标着「未进主线」，而「等你落笔」里一个字都没有。
  // 徽标是**被动的**——它标在那儿，不会告诉你该干什么。而这笔债只有人能了结
  // （建一张 integrator 单去合，或者手工合）。
  const r = 闸表.等我({ 工单表: [
    单('A', '质检', { 待集成: { 时刻: 早, 说: '项目主工作区有未提交改动，拒绝发布', 分支: 'platform/x/A' } }),
    单('B', '已归档', { 待集成: { 时刻: 早, 说: 'x', 分支: 'y' } }),
  ] });
  assert.equal(r.计数, 1, '归档的不再管；质检中但没进主线的仍然欠着');
  assert.equal(r.债[0].闸号, 'P13');
  assert.ok(r.债[0].title.includes('分支 platform/x/A'), '要点名是哪个分支——不然人不知道去合什么');
  assert.match(r.债[0].按钮, /integrator/);
});

t('质检也进在跑清单——判官跑十六分钟不能对平台隐形（协-027）', () => {
  // 2026-08-24 实测：7 个 claude 进程正在判 HW-4、审阅工作区也在，
  // 而 执行器态.在跑 是 []、因 还写着「跑完 HW-4」——平台以为自己闲着。
  const 源 = fs.readFileSync(path.join(平台根, 'scripts', '执行器.js'), 'utf8');
  assert.ok(源.includes("记开跑(id, '质检'"), '质检开跑要记');
  assert.ok(源.includes("记跑完(id, '质检'"), '质检跑完要销');
  assert.ok(源.includes("记开跑(id, '执行'"), '执行那条照旧');
  assert.ok(源.includes('在跑单 = new Map()'), '光有单号说明不了「它在干什么」——执行与质检的处置完全不同');
  assert.ok(!源.includes('已跑毫秒: Date.now()'),
    '别在写盘时算已跑多久——写盘只发生在进出清单的两个瞬间，那个数一落盘就冻住，读到的永远是 0');
});

t('被中断的质检也要进欠账（它停在「质检」不是「在途」）', () => {
  const r = 闸表.等我({ 工单表: [
    单('A', '质检', { 中断: '服务停机中断了质检', 中断类别: '质检', 中断于: 早 }),
    单('B', '在途', { 中断: '服务停机中断了执行', 中断类别: '执行', 中断于: 早 }),
  ] });
  assert.equal(r.计数, 2, '只看在途的话，被中断的质检查无对证');
  assert.ok(r.债.every((x) => x.闸号 === 'P6'));
  assert.ok(r.债.find((x) => x.id === 'A').title.includes('质检'), '要说清被中断的是哪一类——两者点的按钮不同');
});
