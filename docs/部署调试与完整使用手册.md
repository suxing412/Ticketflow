...
| 监制台运行目录 | `C:\Users\你\Desktop\AI工作室` | `studio.config.json`、工单状态目录、回执、流水、角色协议 |
...
如果使用 pnpm，必须允许 Electron 的安装脚本，否则 Electron 二进制不会下载。当前项目最省事的方式是使用 npm。
...
如果当前 Codex 工作区已经安装过项目依赖，可以直接使用 Codex 自带 Node，无需再次执行 `npm install`：

```powershell
$codexNode = 'C:\Users\Sanxing\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe'
$env:STUDIO_ROOT = Join-Path $env:USERPROFILE 'AIStudioDev'
Set-Location 'C:\Users\Sanxing\Documents\GitHub\AIworkflow-GameStudio\_app'
& $codexNode .\server.js
```
...
上述备用路径是当前 Codex 桌面环境的本机路径，不应写进项目配置或分发脚本；换机器后应重新安装 Node.js/npm。
...
当前 `package.json` 把输出目录配置为：

```text
D:\studio-build\dist
```

如果机器没有 `D:` 盘，先修改 `_app/package.json` 中的：

```json
{
  "build": {
    "directories": {
      "output": "C:/studio-build/dist"
    }
  }
}
```

同时修改 `套件/打包套件.ps1` 中查找 EXE 的路径，否则下一步找不到构建产物。
...
```text
监制台-套件-v<version>.zip
```

打包脚本会重新生成干净骨架，不会把当前开发环境的工单、回执、凭据或风格库内容打进分发包。
...
实弹 Orchestrator 会被要求输出一个 JSON 计划。核心结构：
...
Provider 成功输出只保留尾部作为回执，失败原因也会截断。如果需要完整厂商日志，应在厂商 CLI 或本地包装脚本中开启日志，不要把密钥写进日志。
...
```powershell
git -C 'C:\AIStudioDev\workspaces\MyProduct\APP-1-1' status
git -C 'C:\AIStudioDev\workspaces\MyProduct\APP-1-1' log --oneline --decorate -10
```
...
- Provider 完整 stdout/stderr 不全部持久化，深度排查时需要厂商 CLI 日志或包装脚本。
