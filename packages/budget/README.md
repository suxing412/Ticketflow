...
- **不同 Provider 的 token 不宜横向比较**：有的把仓库上下文计入 input，有的把大部分读取归到 cache；同一个 `日token` 在不同池代表的可跑次数可能相差一个数量级。订阅池应以厂商窗口为主，token 只适合作为读数盲区时的兜底。
- 是否能计量不按池名判断：只要本次 JSONL 携带 `usage` 就会入账；没有携带时该次不会入账，调用方应明确提示。
