...
// 守护（塔 → 终端）：三方互保的第三条边。
// 制作人 2026-08-29 03:29 拍板「塔／监制台／终端三方互保，一个环 + 三个外部起点」。
// 终端那侧已有 终端→塔、终端→监制台 两条边（server/lib/互保.js），这里补最后一条。
//
// **塔不自己 spawn exe，而是去触发那条已注册的计划任务。** 两个理由：
//   ① 启动细节（哪个 exe、工作目录、portable 注入的 env）只该有一处正本。塔再写一份，
//      就是两处各自会坏、坏法还不一样——正是终端 互保.js 头注的告诫。
//   ② 任务是终端 --install 亲手注册并验过的，指的必然是部署区那份；
//      塔去猜 exe 路径，换装后必然猜错（本机现成的反面教材：启动监制台.vbs 曾写死 0.37.0）。
// 代价：终端换装后要重跑一次 --install 刷新任务。这一条写进换装仪式。
const 默认守护 = {
  启用: true,
  间隔毫秒: 120000,          // 比心跳（5 分钟）快，比终端自己的互保（60 秒）慢——塔是后备不是主力
  目标: [{
    名: '终端',
    任务名: '游戏开发者终端',
    探址: 'http://127.0.0.1:4280/health',
    超时毫秒: 5000,
    连续失败阈值: 2,        // 一次探不通不算死，可能只是它正在重启
  }],
  // 自启核验：三条自启任务指向的路径还在不在。
  //
  // **换装最容易悬空的就是这里，而且悬空是静默的。** 本机现成的案子：
  // 启动监制台.vbs 里写死「监制台 0.37.0.exe」，那个 exe 早被换装删掉；
  // 计划任务照跑、照报 Last Result: 0，什么也没启动——2026-08-29 03:31 那次自启就是这样空转的，
  // 直到 03:37 监制台死了没人拉才暴露。任务「在册」和任务「拉得起来」是两回事。
  // 终端那条现在指的是带版本号的 exe，下次换装若忘了重跑 --install 就会犯同样的病。
  // 所以要有人定期去看一眼，把静默失效变成看得见的。
  自启核验: {
    启用: true,
    间隔毫秒: 3600000,      // 一小时一次就够：这东西只在换装那一刻变坏
    任务名: ['瞭望塔', '监制台', '游戏开发者终端'],
  },
};

// 从 schtasks /Query /FO LIST /V 的输出里拆出「真正会被执行的那个文件」。纯函数，可单测。
// 两种形态都要认：
//   直接指 exe   → D:\...\游戏开发者终端 0.7.2.exe
//   经 wscript   → C:\WINDOWS\System32\wscript.exe "D:\...\启动.vbs"   ← 要的是引号里那个
// 回 { 在册, 路径 }；拆不出路径时 路径 为 null（**不许猜**，猜错会报假警）。
function 拆任务路径(输出) {
  const 行 = String(输出 || '').split(/\r?\n/)
    .find((l) => /^\s*(Task To Run|要运行的任务)\s*:/.test(l));
  if (!行) return { 在册: false, 路径: null };
  const 值 = 行.replace(/^\s*(Task To Run|要运行的任务)\s*:\s*/, '').trim();
  if (!值 || /^N\/A$/i.test(值)) return { 在册: true, 路径: null };
  const 引 = 值.match(/"([^"]+)"/);
  if (引) return { 在册: true, 路径: 引[1] };
  // 没引号：整行就是路径（含空格也照收，schtasks 不会在后面接参数而不加引号）
  return { 在册: true, 路径: 值 };
}
// 打断表：哪些信箱类型**不该打断人**。
//
// 病灶（2026-08-29 实测）：信箱 544 行里急占 64%，而急件最大的三块——
// 待审 136 / OAuth续命 47 / 人闸逾期 36，合计 63%——全是「别处已有真相的副本」：
// 待审的真相在工单库（它是十二态之一的状态变更通知），OAuth 的在凭据文件，
// 人闸逾期的在债模型（它自己就算 停摆小时）。三分之二都是急，通知就贬值了，
// 人学会忽略它，真正该打断的（执行失败、代核不过）淹在里面。
//
// **表里列的是「不打断的」，没列到的一律打断。** 这个方向是刻意的：
// 异厂评审 K1 打过一个反向设计（默认不打断）——发信端可发 35 种类型，
// 544 行只出现过 26 种，另外 11 种今天就写在 shipped 代码里、可达、25 天零触发，
// 其中 9 种源码硬编码为急（台账损毁 / 依赖成环 / 同号双态 / 巡检连炸 / 裁决异常 /
// 派发滞留 / 跨计费降级 / 零输出 / 额度报警）——恰好是危险度最高的一档。
// 默认不打断的话，它们首次触发就被静默降级。**所以失效方向必须朝响。**
//
// 本表**只做减法不做加法**：命中即压制弹通知，未命中一概不干预（照规则表原样走）。
// 这同时解决了评审点出的另一处：规则表首匹配会让降级件下坠命中「三振上呈」那条
// 通配规则（信源=*、正则 三振|上呈|待裁|代核不过）继续弹窗——压制放在派发层，
// 跟命中哪条规则无关，绕不过去。
//
// 压制的只是**弹通知**；记流水与记未读照旧。所以不是把事情丢掉，是不打断。
const 默认打断表 = {
  不打断: [
    '待审',        // 工单十二态之一的状态变更通知，真相在工单库
    '人闸逾期',    // 债模型自带 停摆小时；且与「滞留告警」在同一粒债上报同一条件，留后者响
    '专项待签', '专项待关账',   // 债模型
    'OAuth续命',   // 终端凭据格已显示剩余分钟
    '编辑器占用', '编辑器锁',   // /api/editor-lock
    '收口报告', '初检不过', '候引擎实证', '台账孤粒',  // 流水足够，不必打断
  ],
  // 注：「巡检异常」不列。它今天本就是常级、不弹通知，列进来不构成降级，
  // 且评审指出它那一行给不出真相住址——不满足「降级必须说得出住址」这条，故不降。
};
...
const 默认规则表 = {
  ...
  规则: [
    ...
    // 守护三条。**扶不动那条是急件**：能扶起来只是掩盖，扶不动才是真要人看的时候。
    // 「扶起」也要弹通知——自动重启而不报告比手动重启加告警更坏：
    // 「一直好好的」和「死了十次又被扶了十次」在屏上长得一模一样。
    { 名: '守护扶不动', 信源: '守护', 正则: '扶不动|停手', 级别: '急', 动作: ['记流水', '记未读', '弹通知'] },
    { 名: '守护扶起', 信源: '守护', 正则: '已触发自启', 级别: '急', 动作: ['记流水', '记未读', '弹通知'] },
    { 名: '守护归位', 信源: '守护', 正则: '归位', 级别: '常', 动作: ['记流水', '记未读'] },
...
function 规范信箱(行) {
  ...
  // 类型 要带出去：打断表按它查。**不能从 文本 里正则抠**——
  // 摘要正文里嵌了别的类型名的情况实测存在（22 条 OAuth 摘要含「判官席空烧三振」），
  // 抠出来的会是错的。这里拿的是结构化字段，不会撞词。
  return { 原时刻: j.t || '', 文本: 片.join(' '), 级别: j['级别'] || '', 类型: j['类型'] || '' };
}
...
// ——————————————————————————————————————————————————————————
// 守护判定（纯函数，可单测）：给一个状态与一次探测结果，回该做什么。
//
// **抽成纯函数不是为了好看。** 退避表是 [0, 30 秒, 2 分钟]，端到端要验「连三次停手」
// 得等 150 秒以上；而这三条闸恰恰是装错了会把系统搞坏的地方（无限重启、端口撞车），
// 不能因为难测就不测。形制照 心跳判定（U6）。
//
// 回 { 动: '无' | '观望' | '扶' | '归位', ... }
//   观望 = 端口有人听但不应答。**这不是死**，可能只是卡住，起第二个会撞端口两个都废。
//   扶   = 端口没人听且连续失败达阈值且不在退避中。
function 守护判定(状态, 探, 阈值, 退避表, 此刻) {
  const 阈 = Number(阈值) || 2;
  const 表 = (Array.isArray(退避表) && 退避表.length) ? 退避表 : [0, 30000, 120000];
  const 上限 = 表.length;

  if (探.通) {
    // 只在「之前真出过事」时才报归位，否则每轮都在刷「它还活着」
    const 要报 = !!(状态.次数 || 状态.连续失败 >= 阈);
    状态.连续失败 = 0; 状态.次数 = 0; 状态.停手 = false;
    return { 动: 要报 ? '归位' : '无' };
  }

  状态.连续失败 += 1;

  if (!探.没人听) return { 动: 状态.连续失败 === 阈 ? '观望' : '无', 因: 探.因 };
  if (状态.连续失败 < 阈) return { 动: '无', 因: '一次探不通不算死，可能正在重启' };
  if (状态.停手) return { 动: '无', 因: '已停手' };

  const 等 = 表[Math.min(状态.次数, 上限 - 1)];
  if (状态.上次 && 此刻 - 状态.上次 < 等) return { 动: '无', 因: '退避中' };

  状态.次数 += 1;
  状态.上次 = 此刻;
  if (状态.次数 >= 上限) 状态.停手 = true;
  return { 动: '扶', 次数: 状态.次数, 到顶: 状态.次数 >= 上限, 因: 探.因 };
}
...
function 载规则(环境) {
  ...
  const 守护基 = Object.assign({}, 默认守护, 基['守护'] || {}, 环境.配置['守护'] || {});
  // 目标必须是数组且每项要有 任务名 + 探址，缺一样就不是个能用的目标——静默丢掉比留着假目标强，
  // 但要留痕：假目标会让「守护在跑」这句话成立而实际什么都没守。
  守护基['目标'] = (Array.isArray(守护基['目标']) ? 守护基['目标'] : []).filter((t) => {
    const 好 = t && String(t.任务名 || '').trim() && String(t.探址 || '').trim();
    if (!好) 警告.push(`守护目标缺 任务名 或 探址，已丢弃：${JSON.stringify(t)}`);
    return 好;
  });
  const 单仓 = 远端基['仓清单'];
  ...
  return {
    ...
    // 打断表：与上面几段同口径带内置回落。**这个 return 是白名单式的**——
    // 不在这里列出来的顶层键会被整个丢掉，于是「与规则并列、同一套热更」那句话就是假的。
    // 异厂评审在驳回别人时顺带查出这一点（零条正面立案），实测属实，故显式接住。
    // 回落方向：JSON 里没有或写坏了，回到内置表（＝今天的降噪表），而不是回到「什么都不压制」——
    // 后者会让配置一坏就悄悄变回原来的吵，属于静默失效。
    打断表: (基['打断表'] && Array.isArray(基['打断表'].不打断)) ? 基['打断表'] : 默认打断表,
    警告,
...
function 守望(环境, 参数) {
  ...
  const 压制已报 = new Set();          // 打断表与规则表冲突时，每个类型只提示一次
  function 派发(信源, 规则, 文本, 原时刻, 类型) {
    const 动作 = Array.isArray(规则.动作) ? 规则.动作 : ['记流水'];
    const 级别 = 取值(规则.级别, '常');
    const 名 = 取值(规则.名, '(无名)');
    // 打断表只对信箱生效，且只做减法：命中即不弹通知，未命中一概不干预。
    // 放在这里而不是靠改规则行——规则表首匹配会让降级件下坠命中通配规则继续弹窗。
    const 压制 = 信源 === '信箱' && !!类型
      && Array.isArray(规则表.打断表 && 规则表.打断表.不打断)
      && 规则表.打断表.不打断.includes(类型);
    ...
    if (动作.includes('弹通知') && !参数['no-toast']) {
      if (压制) {
        // **静默否决一条显式规则是不能接受的。** 有人在规则表里写了「这条要弹」，
        // 打断表却把它压掉——不留痕的话，那个人会以为自己改错了规则，去改一个没坏的东西。
        // 两张表都在 规则.json 里、由同一个人维护，所以冲突是配置错误，该看得见。
        // 每个类型只报一次（进程内），否则 233 条压制就是 233 行噪声——那是另一种吵。
        if (!压制已报.has(类型)) {
          压制已报.add(类型);
          记流水('守望', '常', '打断表压制',
            `规则「${名}」本要为类型「${类型}」弹通知，被打断表压住了。`
            + `两者都在 规则.json 里：要让它响，从 打断表.不打断 里删掉「${类型}」。本类型只提示这一次。`);
        }
      } else {
        弹(信源, 级别, 名, 文本);
      }
  ...
  // ——————————————————————————————————————————————
  // 信源⑥ 守护（塔 → 终端）：三方互保的第三条边
  //
  // 三条闸，缺一条这套东西就从帮手变成祸害（与终端 互保.js 同一套口径）：
  //   一 · 先确认真死了再扶。**ECONNREFUSED＝端口没人听＝真死；超时＝有人听但不应答**，
  //        可能只是卡住，这时再拉一个会撞端口，两个都废——所以超时一律不判死。
  //        这条比 netstat 更准：它问的是「这个服务应不应」，不是「这个端口有没有人占」。
  //   二 · 退避 + 上限。起不来的东西会被无限重启。1 次立刻、2 次 30 秒、3 次 2 分钟，
  //        连三次不成就停手，把话摆到未读上等人看。
  //   三 · 每次扶都留痕并且要看得见。**价值不在自动重启，在「有人知道它死过」。**
  const 守护退避 = [0, 30000, 120000];
  const 守护上限 = 守护退避.length;
  const 守护态 = new Map();
  // **键必须唯一标识「这个目标」，不是「这个任务」。** 首版拿 任务名 当键，
  // 判据当场抓到：两个目标指同一个任务名就共用一份状态，卡住的那个把 连续失败 加上去，
  // 在岗的那个一看「连续失败 ≥ 阈」就报了归位——一个目标的病写进了另一个目标的账。
  const 守护键 = (目) => `${目.名 || ''}|${目.探址}|${目.任务名}`;
  const 取守护态 = (目) => {
    const k = 守护键(目);
    if (!守护态.has(k)) 守护态.set(k, { 连续失败: 0, 次数: 0, 上次: 0, 停手: false });
    return 守护态.get(k);
  };
  let 守护下次 = 0;
  let 守护在途 = 0;
  ...
  function 探守护(目, 回) {
    const 超时 = Number(目.超时毫秒) || 5000;
    let 收线 = false;
    const 收 = (v) => { if (!收线) { 收线 = true; 回(v); } };
    let req;
    try {
      req = http.get(目.探址, { timeout: 超时 }, (res) => {
        res.resume();
        res.on('end', () => 收({ 通: res.statusCode === 200, 因: `HTTP ${res.statusCode}`, 没人听: false }));
      });
    } catch (e) { return 收({ 通: false, 因: (e && e.message) || '请求起不来', 没人听: false }); }
    req.on('timeout', () => { try { req.destroy(); } catch { /* 已断 */ } 收({ 通: false, 因: '超时', 没人听: false }); });
    req.on('error', (e) => 收({ 通: false, 因: (e && e.code) || '连不上', 没人听: (e && e.code) === 'ECONNREFUSED' }));
  }
  ...
  function 巡守护(配) {
    for (const 目 of (配['目标'] || [])) {
      ...
      探守护(目, (r) => {
        ...
        const st = 取守护态(目);
        const 名 = 目.名 || 目.任务名;
        // 三条闸全在 守护判定 里，这里只负责「按结论办事 + 留痕」。
        // 判断与副作用分开，退避与上限才测得动（端到端等 150 秒是测不动的）。
        const 判 = 守护判定(st, r, 目.连续失败阈值, 守护退避, Date.now());

        if (判.动 === '归位') {
          const 文本 = `${名}归位：${目.探址} 已回 200`;
          派发('守护', 匹配规则(规则表.规则, '守护', 文本, 规则警告), 文本, '');
          return;
        }
        if (判.动 === '观望') {
          记流水('守望', '常', '守护观望', `${名} ${判.因}——端口有人听但不应答，不许起第二个实例`);
          return;
        }
        if (判.动 !== '扶') return;

        // 闸三：动手即留痕，成败都有话
        const r2 = 跑(`schtasks /Run /TN "${目.任务名}"`);
        const 文本 = r2.码 === 0
          ? `${名}失守，已触发自启（第 ${判.次数} 次）：${判.因}`
          : `${名}扶不动：schtasks /Run 退出码 ${r2.码}（${String(r2.出).slice(0, 120)}）`;
        派发('守护', 匹配规则(规则表.规则, '守护', 文本, 规则警告), 文本, '');

        if (判.到顶) {
          const 停 = `${名}连拉 ${守护上限} 次都没起来，停手等人看——扶得起来只是掩盖，扶不动才是要查的`;
          派发('守护', 匹配规则(规则表.规则, '守护', 停, 规则警告), 停, '');
        }
      });
    }
  }

  // 自启核验：一小时一次，看三条自启任务指向的文件还在不在。只报变化，不刷屏。
  let 核验下次 = 0;
  const 核验已报 = new Map();          // 任务名 → 上次报过的结论，同结论不重复报
  function 巡自启核验(配) {
    const 好名 = []; const 坏名 = [];
    for (const 名 of (配['任务名'] || [])) {
      const r = 跑(`schtasks /Query /TN "${名}" /FO LIST /V`);
      let 结论; let 详; let 指向 = null;
      if (r.码 !== 0) {
        结论 = '不在册'; 详 = `自启任务「${名}」不在册——冷启动这条腿是断的`;
      } else {
        const t = 拆任务路径(r.出);
        if (!t.路径) { 结论 = '读不出'; 详 = `自启任务「${名}」在册，但读不出它要跑的文件`; }
        else if (!fs.existsSync(t.路径)) {
          结论 = '悬空'; 详 = `自启任务「${名}」指向一个不存在的文件：${t.路径}——它会照跑照报成功，实际什么也不启动`;
        } else { 结论 = '好'; 指向 = t.路径; 详 = `自启任务「${名}」指向 ${t.路径}`; }
      }
      // 小结里要带**指向哪个文件**，不能只带好坏计数。
      // 2026-08-29 无人值守唤醒实测里，被唤起的坐席点出了这个缺陷：
      //   「日志只打好坏计数；路径详情在 核验已报 Map 里被判重了，只在结论变化时才写。
      //     全好 = 一整天不打印路径。所以我知道『三个都存在』，不知道『存在的是哪三个』——
      //     换装有没有指向旧版本 exe，这条日志答不了。」
      // 它是对的：只查「文件在不在」漏掉了「在的是不是该在的那个」，
      // 而那正是 监制台 0.37.0 那桩事故的形状（文件在过，换装后没了；反过来同理）。
      // 取 basename 而不是全路径：版本号在文件名里，够看出漂移，又不把流水行撑爆。
      (结论 === '好' ? 好名 : 坏名).push(结论 === '好' ? `${名}→${path.basename(指向 || '?')}` : 名);

      if (核验已报.get(名) !== 结论) {
        const 首次 = !核验已报.has(名);
        核验已报.set(名, 结论);
        if (结论 === '好') {
          if (!首次) 记流水('守望', '常', '自启核验', '转好：' + 详);   // 从坏变好值得单报一句
        } else {
          派发('守护', 匹配规则(规则表.规则, '守护', 详, 规则警告), 详, '');
        }
      }
    }
    // **每轮都留一行小结，哪怕全好。** 不留的话「检查在跑」与「检查根本没起来」
    // 在流水里长得一模一样——这正是本功能要治的那种沉默。一小时一行，不算噪声。
    记流水('守望', '常', '自启核验',
      `好 ${好名.length}（${好名.join('、') || '无'}）· 坏 ${坏名.length}${坏名.length ? '（' + 坏名.join('、') + '）' : ''}`);
  }
  ...
  function 一轮() {
    ...
    for (const 行 of r2.行) {
      ...
      派发('信箱', 匹配规则(规则表.规则, '信箱', e.文本, 规则警告), e.文本, e.原时刻, e.类型);
    }
    ...
    // 信源⑥守护（塔 → 终端）
    const 守护配 = 规则表.守护 || 默认守护;
    if (守护配['启用'] !== false && Date.now() >= 守护下次) {
      守护下次 = Date.now() + (Number(守护配['间隔毫秒']) || 120000);
      巡守护(守护配);
    }

    // 信源⑥之二：自启核验（一小时一次，只报变化）
    const 核验配 = 守护配['自启核验'] || 默认守护.自启核验;
    if (守护配['启用'] !== false && 核验配 && 核验配['启用'] !== false && Date.now() >= 核验下次) {
      核验下次 = Date.now() + (Number(核验配['间隔毫秒']) || 3600000);
      巡自启核验(核验配);
    }
  ...
  if (参数.once) {
    ...
    const 起 = Date.now();
    const 等收 = () => {
      // 守护也要等：不等的话 --once 会在探测回来之前就退，实测里守护那一格永远是空的
      if ((远端在途 || 守护在途) && Date.now() - 起 < 30000) return void setTimeout(等收, 200);
...
module.exports = {
  ...
  新尾随, 尾随读, 心跳判定, 守护判定, 拆任务路径, 默认守护, 默认打断表, 时钟到点,
  写心跳戳, 读心跳戳, 默认心跳戳间隔,
